...
Setup is in the [Quickstart](/docs/start/quickstart). The four operations
are defined in [Assured operations](/docs/reference/assured-operations).
...
| | `time` | `{"mode": "current"}` | see [Ask about the past](/docs/guides/ask-about-the-past) |
...
ambiguity](/docs/guides/unknowns-and-ambiguity). An anchor that is not a current
...
unknowns and ambiguity](/docs/guides/unknowns-and-ambiguity) has a system-prompt
...
| Read a claim's `claim_valid_from`/`claim_valid_until` as proof that something held at a date. That window is the source's statement. | Ask `facts_context` with `time` set, and read the fact's `validity`. See [Ask about the past](/docs/guides/ask-about-the-past). |
...
| Take the first of several `resolve_entity` candidates. | Ask which one is meant, rank them with context, or pass them all as `entity_ids`. See [Handle unknowns and ambiguity](/docs/guides/unknowns-and-ambiguity#names-that-match-more-than-one-entity). |
...
| Ask about a document right after ingesting it and conclude it says nothing. | Wait until [readiness](/docs/guides/wait-for-readiness) reports `ready`. |

## Next

- [Ask about the past](/docs/guides/ask-about-the-past)
