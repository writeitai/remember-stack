...
Setup is in the [Quickstart](/docs/start/quickstart). The two clocks are
explained in [Time](/docs/concepts/time).
...
`claims_live` with `asserted_at`; see [Explore memory with SQL](/docs/guides/sql).
...
([Explore memory with SQL](/docs/guides/sql)). `facts_as_of` takes a world-time
...
It picks the facts the memory held on the belief instant, but shows each
with its dates as they stand today: a window corrected after that instant
