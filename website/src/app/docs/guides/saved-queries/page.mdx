...
space as any [SQL query](/docs/guides/sql): prepared, read-only views and functions,
with every statement checked before it runs.

Setup is in the [Quickstart](/docs/start/quickstart).
...
The result is subject to the same [limits](/docs/guides/sql#5-limits) as any SQL
...
- [Ask about the past](/docs/guides/ask-about-the-past): `claims_as_of` and
  `changed_since` in use.
- [Cite the source of an answer](/docs/guides/cite-sources): `explain` in use.
