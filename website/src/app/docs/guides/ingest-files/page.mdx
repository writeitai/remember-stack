...
| `parked` | `"no_route"` when the deployment has no converter for the file's MIME type: the bytes are stored but will not be read until an operator adds one. `None` when processing was scheduled. |
