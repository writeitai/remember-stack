...
[What the first ingest fixes](/docs/reference/http-api/ingest#what-the-first-ingest-fixes).
...
Every ingest is stored. Whether it is read depends on the MIME type<Cloud> and on
where you run</Cloud>.
...
remember.dev's text checks add three more: 413 `source_bytes_limit_exceeded`
for more than 10,000,000 bytes (split the file), 409
`rate_class_unavailable` for binary content, and 422 `rate_class_ambiguous`
or `empty_text`.
