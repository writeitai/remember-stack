...
| `parked` | `"no_route"` when the file's conversion is parked waiting for a conversion route for its MIME type: the bytes are stored but not read until an operator adds a route if needed and runs `remember ops resume-no-route`. `None` means only that it is not parked for `no_route`; readiness tells you the processing state. |
