...
Remember Cloud is the managed platform being built around the open-source Remember engine. It provides the same open-source engine and bitemporal store as self-hosted instances, but removes database administration, storage maintenance, worker tuning, and backup operations.

Org / project / deployment and keys: [How Remember Cloud works](/docs/cloud/how-it-works). What the CLI refuses on each backend: [Managed vs self-hosted](/docs/cloud/managed-vs-self-hosted).
