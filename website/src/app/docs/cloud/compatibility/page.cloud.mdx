...
| `DELETE /documents/{doc_id}` | Added after v0.17.0. Not present on remember.dev yet (`404` with `Not Found`, not `document_not_found`); `remember documents delete` and the `delete_document` MCP tool get the same answer |
| `POST /documents/search` | Added after v0.17.0. Not present on remember.dev yet (`404`); `remember documents search` and the `search_documents` MCP tool of `remember mcp` get the same answer |
...
- [Hosted MCP](/docs/cloud/hosted-mcp) already uses the v0.16.0 names.
...
| How to get one | You create it | Console, API, or `remember login`; see [Tokens and sign-in](/docs/cloud/tokens-and-sign-in) |
...
| Deployment blocked for funding (today; see [Spend caps](/docs/cloud/spend-controls)), or closed | The hostname stops answering |
