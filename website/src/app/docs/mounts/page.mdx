...
`ConsumptionSkillSurface` reads live deployment metadata (name, language, scopes, K page count, active recipe versions) plus published mounts and renders a versioned standard `SKILL.md` (skill revision **2.1.0**).
...
## S58 cold-agent check

Eval **S58** gives a model only the rendered skill + a task. It must:
...
Wrong plans (including claim search as current-truth answers) fail. Runs record against skill revision in `eval_runs`.
