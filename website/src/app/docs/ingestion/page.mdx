...
| **P1 / graph / P3** | Store | Search indexes, live PostgreSQL graph, corpus FS | No completion on query; P3 build only |
...
E0 checks file bytes before storing an upload. The detected file class controls
conversion, metering, and original storage. A declared MIME that contradicts
the bytes returns HTTP 422 (`content_type_mismatch`); unknown binary returns
`unsupported_binary_content`. Markdown, CSV, code, and plain UTF-8 text share
the text class; a Markdown declaration remains a rendering hint. UTF-8 BOM and
CRLF text are accepted. Valid files without a configured conversion route are
stored and parked until that route is available.
...
2. Prefer `facts_context` / `facts_current` for “what do we know now?”
3. Use `claims_and_sources_context` / claims for “what sources said.”
