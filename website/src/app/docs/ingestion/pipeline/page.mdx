...
All stages are **idempotent** with respect to their versioned inputs. Workers use the deployment work ledger: retry, then dead-letter for **systemic** failures (provider outage, database errors, unrecoverable handler bugs). That is not the same as **re-derivable soft drops** inside a successful stage: unknown predicates, signature rejects, and **unknown entity types after the E3 type-gate retry budget (D86)** are dropped with structured logs/metrics (`e3.unknown_entity_type*`) and do not dead-letter the document version. Soft drops are intentional and instrumented — not silent.
...
| T1 | Trigram / fuzzy |
| T2 | Phonetic |
| T3 | Embedding similarity |
| T4 | LLM adjudication (gated) |
...
Bare head nouns (`game`, `app`, `system`, `card`, `photo`, `module`, `the system`) are not minted as entities. Qualified names (`FIFA 23`, `James's Unity strategy game`) may mint. When the claim spelling differs from the canonical name (`App` vs `Application`), both surfaces are stored as aliases on the same id.
