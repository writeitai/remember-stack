...
| T4 | Tri-state LLM adjudication (`same` / supported `different` / `insufficient_evidence`) for empty/conflicting or multiple candidates, with candidate profile + current salient facts |
...
Write-time T0 never accepts an exact name by itself. A supported T4 `different`
may mint a second active entity with the same normalized lemma and stores that
pair as a clustering exclusion. Insufficient evidence, an unchecked candidate,
or a truncated candidate prefix may mint only a provisional, merge-eligible
fragment and never creates an exclusion. Lemmas that span multiple entity ids are
marked promiscuous and rank below unguarded fuzzy/phonetic matches without
being dropped from the candidate set. The shipped cascade has no exact-name
auto-accept flag.
