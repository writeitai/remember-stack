...
| T4 | Gated LLM adjudication for empty/conflicting or multiple candidates, with candidate profile + current salient facts |
...
Write-time T0 never accepts an exact name by itself. A T4 non-match may mint a
second active entity with the same normalized lemma; the negative pair is
stored as a clustering exclusion. Lemmas that span multiple entity ids are
