...
unless the deployment trusts their network source and the caller has full
`write` authority. The unscoped `API_BEARER_BIND` shared secret is unrestricted
and qualifies; only narrow signed credentials are refused.
