...
Each entry must be an **exact** `https://` origin — `https://app.example.com`,
not `https://*.example.com`, not a path, not `http://`. Anything else is
refused at startup rather than narrowed silently, because a wildcard that
looks accepted is worse than one that fails loudly.
