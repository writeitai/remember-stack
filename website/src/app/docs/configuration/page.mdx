...
### Lance index maintenance (D93)

Compose runs `rememberstack self-host maintain-p1` as `worker-maintain-p1`. The
loop is off until you flip a gate. Writers keep inserting while it runs; the
ticker try-locks only against purge and another maintain pass.

| Variable | Purpose |
| --- | --- |
| `REMEMBERSTACK_P1_MAINTAIN_MAINTENANCE_ENABLED` | Run ensure/compact. Default `false` |
| `REMEMBERSTACK_P1_MAINTAIN_HEAVY_ENABLED` | Also evaluate IVF retrain. Default `false`. Do not enable until compact/ensure have been soaked |
| `REMEMBERSTACK_P1_MAINTAIN_POLL_S` | Sleep between ticks (default 60) |

Physical Lance files live on the Compose `app-state` volume at
`/var/lib/rememberstack/lance`.
