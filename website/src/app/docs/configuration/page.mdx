...
Credentialed CORS is disabled (`allow_credentials` is off), so the browser
will not attach cookies or TLS client certificates to these requests and will
reject a response that tries to use them. The browser credential travels in
the `Authorization` header instead. This is a statement about what CORS
negotiates, not a claim that cookies can never exist — same-origin requests
are unaffected.
...
local https listener. Several origins are comma-separated, and the
