...
what that does: it stops the *response* being shared with a page that asked
for credentials, which means an authenticated call is refused before it
reaches a route — a request carrying `Authorization` needs a preflight, and
the preflight fails. It does not reach into the browser and stop a page
...
A port is part of an origin and may be included:
`https://app.example.com:8443` (1–65535). `https://localhost` and
`https://localhost:3000` are accepted, so a self-hoster can develop against a
local https listener.

Validation is not a list of banned shapes: the value is parsed and
re-serialized the way a browser serializes an origin, and it must come back
identical. Anything non-canonical fails that comparison without anyone having
had to think of it first — an uppercase host, a trailing slash, a default port
written out. What parsing cannot decide, such as whether the host is a real
host rather than `*` or a broken IPv6 literal, is checked on top. Several origins are comma-separated, and the
whole list is validated together — a stray comma is a refusal, not a silently
skipped entry.
