...
| Variable | Purpose |
| --- | --- |
| `REMEMBERSTACK_SELFHOST_CONVERSION_ROUTES` | JSON object mapping input MIME type to a converter adapter name, e.g. `{"text/markdown": "passthrough", "text/plain": "passthrough", "text/html": "markitdown"}`. Shipped adapter names: `passthrough`, `markitdown`. Setting the variable replaces the whole table (defaults shown are the stock table); a MIME type without a route dead-letters on convert, and an unknown adapter name refuses startup |
