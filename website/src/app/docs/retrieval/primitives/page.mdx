...
| Surface | Call |
| --- | --- |
| Assured ops | `POST /operations/resolve_entity`, `claims_and_sources_context`, `facts_context`, `combined_context` |
...
| **In** | name; the HTTP resolve route also accepts up to 8 context entity ids |
| **Out** | ranked entity candidates (`tier` is `T0`, `T1`, `T2`, or `T3`) |
| **Shipped behavior** | Current-alias **T0 exact** (complete homonym set), then trigram **T1** and phonetic **T2**, then query-string **T3** embedding search. No LLM adjudication. Ambiguity stays ranked. T1–T3 disclose truncation at the write-path blocking width. |
...
| **Out** | scalar records |
| **Job** | Direct store reads |

Relation and observation lookups include facts whose incomplete world dates might
match the requested time, marked `temporal_match: possible`. These are useful
evidence, but do not establish a confirmed temporal count. The strict
`memory_v1.facts_current` view and live graph exclude possible matches.
...
| | |
| --- | --- |
| **Surface** | **Internal** composition (also frozen inside operations like `claims_and_sources_context`) |
...
### Choosing the fact time scope

Use `history` for biographies and achievements: a completed tournament win
remains part of someone's history. Use `current` for what holds now, `at` for a
particular instant, and `overlap` for a bounded interval. History excludes known
future starts; future planning uses an explicit future `at` or `overlap` request.

The library's enumerated `aggregate` accepts the same time selector. Counts and
groups report `count`/`total` for confirmed matches and
`possible_count`/`possible_total` for incomplete-date candidates separately. A
nonzero possible total or truncation prevents an exact-count claim. Timeline
buckets use chosen world dates, with an undated bucket rather than an ingestion
date fallback. `delta_top_entities` remains a system change feed.
...
### `claims_and_sources_context` (evidence)
...
### `combined_context` (`ContextBundle/v2`)

Purely composes the complete `claims_and_sources_context` and `facts_context` responses
