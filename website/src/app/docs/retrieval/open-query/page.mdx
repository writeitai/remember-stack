...
> **Deployment Architecture**: Open SQL and schema discovery are fully supported on Self-Hosted engine deployments (Docker Compose) backed by the AST-validated query sandbox (`pglast 8.x`). For Remember Cloud, planned query space parity with dedicated per-project database pods and spend-lease metering is actively being operator-dogfooded. See the [compatibility profile](/docs/project-status) for published route status and deviations.
