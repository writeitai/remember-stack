...
World-time overlap on claims uses the **half-open canonical window**
(`claims_canonical.canon_start` / `canon_end`, or `canonical_bounds(...)`).
A stored day is the whole calendar day; an instant is a non-empty point;
unknown precision has no interval and is counted by precision, not by bounds.
The shipped `examples.claims_as_of` query is the honest pattern.
