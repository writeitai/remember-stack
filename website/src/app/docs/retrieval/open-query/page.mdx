...
| --- | --- | --- |
| **SQL** | Live joins, evidence composition, filters, graph helpers | **Live store views** + nomination/traversal SRFs |
...
> **Deployment Architecture**: Open SQL and schema discovery are fully supported on Self-Hosted engine deployments (Docker Compose) backed by the AST-validated query sandbox (`pglast 8.x`). On Remember Cloud, open query routes are currently unsupported and planned for a future release (see the [Project Status](/docs/project-status/) page for published route status and deviations).
...
Use `facts_context` / `claims_and_sources_context` / `resolve_entity` when you want:
