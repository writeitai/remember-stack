export const metadata = {
  ...
  description:
    "Envelope and ContextBundle/v2 wire contracts: explicit temporal scope, separate authorities, freshness, contradictions, and typed negatives.",
};
...
This page documents the **shipped wire shape** for assured operations (and primitive compositions that emit the same envelope). Open SQL uses `QueryResult/v1` instead; see [Open query](/docs/retrieval/open-query).
...
| `validity` | `{valid_from, valid_until, valid_precision, ingested_at, invalidated_at}` |
| `contradiction` / `contradiction_group` | Live co-members package |
| `support` | `current` \| `withdrawn` |
| `temporal_match` | `confirmed` or `possible` for the requested time scope |

`valid_from` and `valid_until` describe one chosen world interval: its start is
included and its end excluded. `valid_precision` distinguishes calendar units,
instants, explicitly open intervals and unknown dates. A missing boundary is not
permission to assume the fact held at every date. `ingested_at` and
`invalidated_at` describe when the engine believed it, not when it happened.

A `possible` match means incomplete dates have not ruled it out. It is not a
stored dispute flag. Fact-grain operations (`facts_context`, relation and
observation lookups) return possible matches in the same list as confirmed
ones, each flagged; only the published strict SQL view `facts_current` and the
live graph exclude them. For example: "Three wins are dated to 2022; one
additional win has no accepted date." Context results are bounded candidates,
so their length never proves an exhaustive count.
...
| `claim_text`, `source_span`, `char_start`, `char_end` | Text + **origin** grounding (the target-chunk owner) |
| `evidence_spans` | Complete supporting body ranges for the **origin occurrence** identified by `chunk_id` (`{char_start, char_end}` objects in that chunk's representation). Empty when that occurrence list is missing; origin scalars are not substituted as complete support. Current-version remapped positions are on `memory_v1.claim_occurrences_live`. |
| `is_current_testimony` | Currency filter result |
| `asserted_at`, `claim_valid_*` | When the source made the statement, and when the claim says it happened or was true (source-asserted, not system belief) |
...
How `facts_context` binds facts to testimony without blending grains:
...
## ContextBundle/v2: both authorities, still separate

`combined_context` is the sole side-by-side testimony/fact response. It does not
use `grain = composite`, and it does not nest parts inside an envelope:

```text
{
  contract: "ContextBundle/v2",
  claims_and_sources: Envelope,  // complete evidence-grain claims_and_sources_context response
  facts: Envelope       // complete fact-grain facts_context response
}
```
...
Projections may nominate candidates the store rejects. `dropped_by_hydration` keeps ranking honest. Compound structures (paths) drop **as units**.

## Freshness block (shipped field names)

| Field | What it tells the agent |
| --- | --- |
| `pg_live_ts` | Live store clock for confirmed rows |
...
The graph has no separate freshness stamp: SQL/PGQ and recursive helpers read
the same PostgreSQL repeatable-read snapshot as hydration.
...
`temporal_scope` is required and discriminated by `mode`. `facts_context`
...
6. Prefer `fact_evidence` + `evidence_totals` when using `facts_context`.
7. For `ContextBundle/v2`, inspect both complete child envelopes separately.
