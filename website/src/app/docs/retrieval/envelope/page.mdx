...
stored dispute flag. For example: "Three wins are dated to 2022; one additional
win has no accepted date." Context results are bounded candidates, so their
length never proves an exhaustive count.
