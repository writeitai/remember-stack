...
| `evidence_spans` | Complete supporting body ranges for the **selected occurrence** (`{char_start, char_end}` objects). Empty when no occurrence list is selected; origin `char_start`/`char_end` are not substituted as complete support. |
