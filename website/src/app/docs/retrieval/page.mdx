...
from those anchors, walks one P2 hop by default, then searches relation labels
...
remain fact rows and never become graph nodes. The combined scope is capped at
20 entities and the envelope marks graph or fact truncation. Missing or lagging
P2 returns a `boundary`, never a silent anchor-only current answer.
...
P2 traversal is point-in-time: `current` and `at` use neighborhood expansion;
`overlap` and `history` preserve the supplied anchor scope and therefore carry
no P2 freshness stamp.
