...
**Mounts skip step 2 by construction** (snapshot files). Rule: orient on mounts; **verify load-bearing facts in the store**.
...
| “What is true now?” with typed envelope | **`facts_context`** |
| High-recall source context for a question | **`claims_and_sources_context`** |
| Both complete authority views | **`combined_context`** |
| Name → entity | **`resolve_entity`** (exact, trigram, phonetic, embedding) |
...
| --- | --- | --- | --- |
| `resolve_entity` | `name` | entity candidates (envelope packaging) | Ranked identity candidates, never a silent guess |
| `claims_and_sources_context` | `query` (optional `entity_ids`, `k`, `candidate_k`) | **evidence** | Current claims + source chunks only |
| `facts_context` | `query` (optional `entity_ids`, `hops`, `predicate`, `k`, `evidence_per_fact`, `time`) | **fact** | Entity-neighborhood relations and observations + associated testimony |
| `combined_context` | `query` (optional `entity_ids`, `hops`, `predicate`, `time`) | `ContextBundle/v2` | Complete claims-and-sources and neighborhood-aware fact envelopes, kept separate |

Entity IDs are optional. Omit them for deployment-wide retrieval. Testimony
accepts up to 20 confirmed IDs; the neighborhood-aware fact and answer
operations accept up to 19 so the 20-entity combined scope always reserves one
neighbor slot. Resolve ambiguous names first. The operations never guess a name
or silently drop an unknown ID.

For a current or point-in-time read with `entity_ids`, `facts_context` starts
...
| Don’t | Do |
| --- | --- |
| Answer present-tense from `claims_and_sources_context` alone | Verify with `facts_context` / facts |
...
`freshness.pg_live_ts`, …). `combined_context` is the separate
`ContextBundle/v2` wrapper around two complete envelopes. Full wire contract:
...
| Param | Meaning |
| --- | --- |
| `facts_context.time` | World-time selection: `current`, `at`, `overlap`, or `history` under current system belief |
...
| SQL/graph helpers/saved | [Open query space](/docs/retrieval/open-query) |
