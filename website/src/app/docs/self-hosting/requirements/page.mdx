...
**Database connections.** PostgreSQL in Compose keeps its default
`max_connections` of 100. Each engine process opens connections as it
needs them, up to these ceilings:
...
The ceilings of the default stack add up to 23 + 12 × 16 = 215, more than
100. They are ceilings, not what a stack holds: pools open connections only
on demand, and a worker runs one item at a time. Nothing caps the total,
though, so budget against the ceilings when you add replicas or API
processes: each worker replica can add 16 connections and each API process
23, plus any increase in the retrieval and graph pool sizes. [Scaling](/docs/self-hosting/scaling#database-connections)
shows how to raise `max_connections`.
