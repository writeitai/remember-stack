...
`max_connections=300` (the image default is 100). Each engine process opens
...
increase in the retrieval and graph pool sizes. Past 300, raise the limit. [Scaling](/docs/self-hosting/scaling#database-connections)
...
| PostgreSQL `max_connections` | 300 (set in `compose.yaml`) |
