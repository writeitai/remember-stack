...
| `remember ops cost-export --deployment ID` | Print one page of the cost ledger ([Observability](/docs/self-hosting/observability#cost-export)) |
...
not use attempts ([Models and providers](/docs/self-hosting/models#openrouter-routing-and-limits)).
...
is still unrouted stay parked. See [File formats and converters](/docs/self-hosting/converters).
...
the workers enforce it.

Work a budget parked shows as `pending` in `remember ops inspect`, and with
`defer_reason` `budget` in the diagnostic query under
[Troubleshooting](/docs/self-hosting/troubleshooting#see-what-is-waiting-and-why).
The spend itself is in the cost ledger; read it with `remember ops
cost-export` ([Observability](/docs/self-hosting/observability#cost-export)).
...
a hard forget; see [Upgrades and migrations](/docs/self-hosting/upgrades#back-up).

remember.dev operates the pipeline for you; there, work parked by a spend
cap resumes after a credit purchase, see [Spend caps and auto top-up](/docs/cloud/spend-controls).
