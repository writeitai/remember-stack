...
PostgreSQL with `max_connections=300`; the engine does not size itself
against that limit. The ceilings per process are:
...
the sum of its ceilings. The default stack's ceilings add up to 215, within
the limit of 300. When you add replicas or API processes, keep the sum of
their ceilings under the limit, and when PostgreSQL refuses connections
(`too many clients already` in the logs), raise it. The Compose `command` for PostgreSQL is one list, so an
override repeats it whole and adds the new setting:
