...
Each engine process has its own connection pools, and PostgreSQL in Compose
keeps its default `max_connections` of 100. The engine does not size itself
...
the sum of its ceilings. The default stack's ceilings add up to 215. When
you add replicas, keep the sum of their ceilings in mind, and when
PostgreSQL refuses connections (`too many clients already` in the logs),
raise its limit. The Compose `command` for PostgreSQL is one list, so an
