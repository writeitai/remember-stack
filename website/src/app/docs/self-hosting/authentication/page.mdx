...
on **all** network interfaces of the host. On a laptop on a shared Wi-Fi
network, a colleague can reach it. (The object store publishes no host
port.)

To publish the API on the loopback interface only, add to
