...
| `REMEMBERSTACK_MINIO_ACCESS_KEY`, `REMEMBERSTACK_MINIO_SECRET_KEY` | Object-store credentials. The bundled SeaweedFS store uses them as its only S3 identity; Compose refuses to start if either is empty. |
...
By default the API needs no token, and Compose publishes port 8000 on
**every** network interface of the host, not only on `localhost`. Anyone
who can reach that port can read and change the memory. Before the machine
is on a shared network, set a token and limit the published port; see [Authentication and scopes](/docs/self-hosting/authentication).
