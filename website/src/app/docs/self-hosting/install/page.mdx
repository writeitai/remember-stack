...
The last command generates the object-store credentials; `.env.example`
ships none, and Compose refuses to start without them.
...
The example PostgreSQL password in `.env.example` is marked local-only.
PostgreSQL publishes no host port, but replace it anyway before the first
start of any deployment you keep.
...
`127.0.0.1` only: only this machine can reach it. To serve other machines,
