...
A self-hosted deployment does not trace model calls. The `LANGFUSE_*` and
`REMEMBERSTACK_LANGFUSE_*` variables are read only by the project's
benchmark harness; setting them for the API and workers changes nothing.
