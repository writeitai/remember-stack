...
reconciliation, and fact labeling. Compose also starts `worker-maintain-p1`, a
loop (not a ledger stage) that can compact and retrain Lance search indexes.
Both `REMEMBERSTACK_P1_MAINTAIN_MAINTENANCE_ENABLED` and
`REMEMBERSTACK_P1_MAINTAIN_HEAVY_ENABLED` default to **false**, so that process
sleeps until an operator turns a gate on. The one-shot `setup` service applies every
...
The PostgreSQL 18 container mounts its named volume at `/var/lib/postgresql`.
...
`docker compose down --volumes` before starting the PostgreSQL 18 checkout.

`--build` deliberately tests the checked-out source. The public `v0.4.1` release
also ships this Compose file pinned to
`ghcr.io/writeitai/remember-stack:0.4.1`; release consumers use
