...
described in [Assured operation routes](/docs/reference/http-api/operations). For anything
these four do not cover, use the [direct routes](/docs/reference/http-api/entities-and-facts),
[search](/docs/reference/http-api/search), the [graph](/docs/reference/http-api/graph) or
...
their ids. See [Give an agent context](/docs/guides/agent-context).
...
at a past instant, use the [graph routes](/docs/reference/http-api/graph) or the
[`facts_as_of`](/docs/reference/query-space#facts_as_of) SQL function.
...
fact with no known start). See [Time](/docs/concepts/time).
...
The HTTP route [`GET /resolve`](/docs/reference/http-api/entities-and-facts#get-resolve)
...
[`ChunkEvidenceResult`](/docs/reference/result-types#chunkevidenceresult) entries.
...
| Field | Contents |
|---|---|
| `facts` | Up to `k` [`FactResult`](/docs/reference/result-types#factresult) entries: `kind` (`relation` or `observation`), `label`, `validity`, `temporal_match`, `support`, and the other sides of any contradiction. |
...
A [`ContextBundle/v2`](/docs/reference/result-types#contextbundlev2):
...
second is the memory's view. See [Reading a result](/docs/concepts/reading-results).
...
`GET /operations` returns one [`ToolDescriptor`](/docs/reference/result-types#tooldescriptor)
