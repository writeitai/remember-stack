...
`remember` package, runs on your machine next to the agent, and talks to one
deployment<Cloud>: a self-hosted engine or a remember.dev project</Cloud>.
...
`delete_document`, and refuses calls to them. `pipeline_readiness` and
`search_documents` only read, so they stay.
...
1. `ingest`, `pipeline_readiness` and `delete_document`, then
   `search_documents` when the deployment's `GET /deployment` lists it at
   the same tool version as this `remember` package
   (only `pipeline_readiness` and `search_documents` with `--read-only`).
   A deployment that does not answer `GET /deployment` gets no
   `search_documents`.
...
A full server therefore lists 15 tools: 4 + 4 + 7.
...
| `invalid_arguments` | 422 | no | `search_documents` with bad arguments, or a `cursor` the deployment cannot read. | Fix the arguments; start a new search instead of reusing the cursor. |
...
| `code` | `http_status` | `retryable` | When | `agent_action` in short |
|---|---|---|---|---|
| `spend_safety` | as sent, else 403 | no | The deployment refused the work to stay within a spend cap. | Tell the user; do not retry. |
...
| `dispatch_parked` | 423 | no | The work is parked until someone acts. | Stop retrying; notify a person. |

</Cloud>

With `--read-only`, a call to `ingest` returns `isError: true` with the
plain text
