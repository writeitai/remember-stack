...
`parked: "no_route"` means the deployment has no converter for the file's
media type. The bytes are stored, but they are not read until an operator
adds a route. The reply then has `pipeline.status` `parked_no_route`, no
