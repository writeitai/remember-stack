...
| Method | Behaviour |
|---|---|
| `initialize` | Requires `params.protocolVersion` (any string). Answers with `protocolVersion` `2025-11-25`, `capabilities: {"tools": {}}` and `serverInfo: {"name": "remember", "version": "<package version>"}`. |
| `ping` | Answers `{}`. |
