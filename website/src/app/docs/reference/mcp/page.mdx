...
remember.dev also runs a hosted server, at `https://remember.dev/mcp`; the
agent signs in through the browser or sends an API key. The tables in
...
You add this URL to your agent. The agent either opens a browser, where you
sign in to remember.dev and choose the projects and permissions it gets, or
sends an API key as `Authorization: Bearer rmb_…`. Setup per client, the
account tools, limits and errors: [Hosted MCP](/docs/cloud/hosted-mcp).

The hosted server serves the memory tools of this package's catalogue,
unchanged, at the engine version remember.dev runs. It adds a `project`
argument to every memory tool, which it resolves and removes before calling
that project's deployment, and it adds account tools that exist only on
remember.dev.
...
| Works with | A self-hosted engine or a remember.dev project | Every remember.dev project the connection or key covers |
| Transport | stdio, one JSON-RPC message per line; or HTTP with `--transport http` | Streamable HTTP: `POST`, one JSON-RPC message per request, no session |
| Protocol version answered | `2025-11-25` | The requested one of `2025-11-25`, `2025-06-18`, `2025-03-26`; otherwise `2025-11-25` |
| Sign-in | Key from flags, environment or the credential file | OAuth in the browser (PKCE, dynamic registration, consent page), or an API key in the header |
| Configure with | `remember setup` | `remember setup --cloud`, or the agent's "add remote MCP server" setting |
| Memory tools | The catalogue of the installed package | The catalogue of the engine version remember.dev runs |
| `project` argument | Refused (`project_routing_unavailable`) | On every memory tool |
| `ingest` bodies | `text`, `content_base64`, `path` | `text`, `content_base64` |
| Account tools | None | Eleven: projects, keys, members, balance and billing links |
| Read-only mode | `--read-only` | Grant only `memory:read` (and `account:read`) at sign-in or on the key |
| Tool errors | A tool result with `isError: true` and a JSON error object | The same |
