...
| `initialize` | Opens a session: the answer carries an `Mcp-Session-Id` header. Every later request must send it back. |
...
It listens on the loopback address unless you bind it elsewhere. With any
other address, it first asks the engine for `GET /deployment` without a key
and refuses to start unless the engine refuses: it never exposes an engine
that answers without a key. It does not do TLS; put a reverse proxy in
front of it when agents connect from other machines.
...
`--self-hosted` (default `http://127.0.0.1:8000`) or with `--api-url`. It holds
exactly one variable, `REMEMBER_API_URL`. Keys are never written into these
files; the server reads them from the credential file.
...
```bash
claude mcp add remember -e REMEMBER_API_URL=http://127.0.0.1:8000 -- /Users/ravi/.local/bin/remember mcp
```
