...
yourself; only the address and the token differ.</Cloud>
...
It also exports the remember.dev account client and its types:
...
`remember.credentials.CredentialError`, raised by the CLI helpers, lives
outside `__all__`<Cloud>, and so does `remember.device_login.DeviceGrantError`</Cloud>.
See [Errors](#errors).
...
To read your remember.dev organisation's balance, deployment and spend
state, use [`CloudClient`](#cloudclient).
...
| Parameter | Meaning |
|---|---|
| `api_key` | The deployment token. A bare secret<Cloud> (`umc_dp_…`)</Cloud> or a full `Bearer …` value; the client adds `Bearer ` when it is missing. |
...
The client never reads the CLI's credential file<Cloud>, the one `remember login` writes</Cloud>.
...
The remember.dev account client and `remember login` add:
...
| `code` | A machine code, set for SQL and saved-query routes (for example `parse_error`, `quota_exceeded`, `saved_query_not_found`) and for a `429` admission refusal (`rate_limited` or `concurrency_limited`); otherwise `None`. |
| `retry_after` | On a `429`, the `Retry-After` seconds the deployment sent (a `float`); otherwise `None`. The client does not retry by itself. |
...
The remember.dev account client returns these dataclasses:
