...
one they are newest ingested first and `cursor` pages them. Passing `cursor` with a
...
`documents` keeps only claims found in matching document versions (each still
cites the passage it was first extracted from), with the same fields
as [`search_documents`](#search_documents); a filtered search is sent as
`POST /search/claims`. See
...
memory = Client()
hits = memory.search_claims(
    query="Project X",
    channel="bm25",
    documents=DocumentSearchFilters(authors=("alice@acme.com",)),
)
```
...
`documents` keeps passages whose document matches.
