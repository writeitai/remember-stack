...
(a connection failure, `421`, or a `404` that is not the engine's), the
client asks again and retries the request once when the address changed
(every request except creating a connector, which is never repeated).
