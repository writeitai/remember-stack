...
| `DeploymentBuildInfo` | model | [`deployment_build_info`](#deployment_build_info) |
...
| `NegativeKind` | enum | [Result types](/docs/reference/result-types#negativekind) |
...
Inherited unchanged from [`MemoryClient.ingest`](#ingest).
...
Reading a path can raise `OSError`: `FileNotFoundError` when it does not
exist, whether given as `Path` or `str`.
