...
| `parked` | `"no_route"` or `null` | `"no_route"` when the deployment has no converter for the upload's MIME type: the bytes are stored, but conversion waits until an operator adds a route. `null` when conversion was scheduled. |
