...
| `parked` | `"no_route"` or `null` | `"no_route"` when the upload's conversion is parked waiting for a conversion route for its MIME type: the bytes are stored, and conversion waits until an operator adds a route if needed and runs `remember ops resume-no-route`. `null` means only that it is not parked for `no_route`; read processing state from `POST /readiness`. |
