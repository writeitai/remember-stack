...
Belief time selects which facts the memory held (`ingested_at` and
`invalidated_at`). World time is tested against each fact's window as it
stands now: a window corrected after `believed_at` is shown and filtered
corrected.
