...
# RememberStack 0.17.0 CLI Reference
...
The canonical `remember` CLI ships in the lightweight `remember` package on PyPI (`pip install remember` or `uvx remember`).
...
> Organization member management is performed via the cloud console at [`https://remember.dev/app/members`](https://remember.dev/app/members). Running `remember members` prints direct console guidance and exits.
