...
`--read-only` omits `ingest` and `pipeline_readiness` while keeping assured
operations and composed open-query tools. Direct calls to the omitted write
tools fail closed. Default `remember mcp` is unchanged.
