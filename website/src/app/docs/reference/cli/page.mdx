...
This page describes the `remember` CLI (v0.17.3), installed by `pip install remember`.
...
`search` finds documents by name, metadata and text and prints a
[`DocumentSearchPage`](/docs/reference/result-types#documentsearchpage) as
...
```bash
remember documents search "q3 sales" --family office | jq -r '.documents[] | "\(.doc_id) \(.file_name)"'
