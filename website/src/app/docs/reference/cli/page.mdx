...
The canonical `remember` CLI (v0.17.0+) ships in the lightweight `remember` package on PyPI (`pip install remember` or `uvx remember`). Surfaces besides the CLI: [Choose your path](/docs/getting-started/choose-your-path). Org / project / keys: [How Remember Cloud works](/docs/cloud/how-it-works).
...
Ingests a file into the content-addressed store, triggering chunk packing, claim extraction, and bitemporal adjudication:
...
# Run facts_context
remember operations run facts_context --arg query="Who is the lead architect?"
```
...
Fetches surrounding source chunks in document order within a symmetric window (1–2, default 1) around a target chunk:

```bash
remember query adjacent-chunks 57000000-0000-0000-0000-000000000001 --window 1
```
...
- **`remember review`**: Retired. In early versions, review queues held human triage tasks. In modern Remember, contradiction adjudication is fully autonomous (D3/D43/D107) and runs live in the bitemporal store. Running `remember review` prints a helpful migration notice.
...
```bash
remember ops resume-no-route --deployment <uuid>
```

The command reads `REMEMBERSTACK_SELFHOST_CONVERSION_ROUTES`, validates its
