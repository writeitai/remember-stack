...
there is no next page; without one they are newest first and `--cursor` reads
the next page. `--limit` is 1 to 200 (default 20). See
...
```text
remember setup [--self-hosted] [--api-url URL] [--api-key KEY]
...
```text
remember setup --cloud [--api-url URL]
               [--agent {cursor,claude,codex,agy,all}] [--dir DIR] [--dry-run]
```
...
| `--api-url` | none | The engine address. It is written into the agent configuration as `REMEMBER_API_URL`. |
...
`--cloud` configures for remember.dev; sign in with `remember login` first.
It refuses `--api-key`: a remember.dev key comes from `remember login` or
`REMEMBER_API_KEY`. With neither `--cloud` nor `--self-hosted`, on an
interactive terminal, the command asks which backend to use; remember.dev is
the default answer. When standard input is not a terminal, it chooses
`--cloud`.
...
2. With `--self-hosted`, stores the engine address and key in the credential
