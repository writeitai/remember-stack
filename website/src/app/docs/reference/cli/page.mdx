...
except with `--headless` or `--dry-run`. It refuses `--api-key`,
...
Existing configuration files must be valid JSON or TOML; otherwise that
agent is not configured and the command exits with status 1. A failure in
