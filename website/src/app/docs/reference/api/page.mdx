...
Client surfaces other than raw HTTP: [Choose your path](/docs/getting-started/choose-your-path).

> **Query contract.** RememberStack exposes exactly four top-level assured
> operations: `resolve_entity`, `claims_and_sources_context`, `facts_context`, and
> `combined_context`. The
...
- **The shipping query surface is deliberate.** Seven open-query entry points
  ...
  `resolve_entity`, `claims_and_sources_context`, `facts_context`, and `combined_context`.
...
| --- | --- | --- |
| `resolve_entity` | `name` | ranked current entity candidates from exact, trigram, phonetic, and embedding tiers; ambiguity is returned, never guessed away |
...
remains in the store so an audit can tell "forgotten" from "never existed",
...
`adjacent_chunks`, `hydrate_relation`, and connector-management methods. Network and non-success
...
| `GET /chunks/{chunk_id}/adjacent?window=` | fetch surrounding source chunks within a window (1–2, default 1) around a target chunk in document order; returns `chunks[]` as typed **evidence** |
| `POST /chunks/adjacent` | the same adjacent-chunk query with `{chunk_id, window}` in the body |
...
representation, offsets, and source timestamps confirmed by live PostgreSQL.
...
`resolve` returns current survivors. An exact alias match returns every exact
candidate. A miss continues through trigram and phonetic blocking, then profile
embedding; those tiers stop at the write-path blocking width and set
`truncation` when further matches were not returned. Nothing in the list is
chosen for the caller. Supplying focal entities only reorders the candidates
that were returned: a candidate related to more of the supplied entities ranks
first, and each candidate discloses its `context_hits`. Context therefore
