...
exact dynamic value. Observations are searched and returned as facts; they are
not graph nodes. A missing P2 snapshot, a current anchor absent from P2, or a
stale neighbor is a `boundary`. Caps are reported in `truncation`.
