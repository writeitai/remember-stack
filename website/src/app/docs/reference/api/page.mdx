...
Lists the document lineages this deployment holds, most recent activity first.
...
| `status` | filter on the newest version's state: `ingesting`, `converting`, `structuring`, `ready`, `failed`, `deleted` |
...
The cursor carries a **watermark** — the instant the listing began — as well
as the sort position, and every page describes each document as of that
instant. Ordering by "newest activity" means the sort key is mutable: without
the watermark, a document below the cursor that is re-ingested mid-scroll
jumps above it, past a place the reader has already been, and is returned on
no page at all. They finish the list believing they saw everything.
...
**Scale.** The order is computed from the newest version's `ingested_at`,
which is derived rather than stored on the lineage, so no index serves the
final sort: each page costs a scan of the deployment's live documents. That is
fine for the corpus sizes deployments hold today and is *not* fine at the
million-document scale this system targets. Closing it needs a durable
latest-activity sort key on `documents`, maintained on the write path and
indexed — a schema change, tracked separately rather than bolted on here.
