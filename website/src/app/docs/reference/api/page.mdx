...
`version_no`, `status`, `ingested_at`, and `error` when it failed),
`serving`, and `skipped_stages`.
...
`skipped_stages` names the pipeline stages that were deliberately **not run**
for that version, and is empty for almost every document. A skip is neither a
failure nor a success — it is work that was considered and declined — and it
changes what the document can answer. A version whose `extract_claims` was
skipped is `ready` and searchable as text while contributing nothing to claim
search, so a caller reporting only `status` would present it as fine and leave
somebody wondering why it never appears in answers. The skips reported are the
newest version's own; one recorded against a superseded snapshot is not
attributed to it.
...
The cursor is a keyset position, not an offset, so a document ingested while
somebody is paging cannot push another off the end of a page and out of the
listing. It is opaque: a value this API did not issue is a **400** rather than
a silent restart, because restarting would re-serve a page the caller has
