...
descriptors, including their implementation-plan hashes. The stock Compose file
enforces that order by making API and workers wait for the setup service to
...
| `fact_context` | `query`; optional `entity_ids` (1–20 unique UUIDs), `hops` (1–2, default 1), free `predicate` (1–255 chars), `k` (1–30, default 15), `evidence_per_fact` (1–5, default 3), `time` | neighborhood-scoped relations and observations for `current` / `at`; anchor-scoped `overlap` / `history`; supporting and contradicting testimony |
| `answer_context` | `query`; optional `entity_ids` (1–20 unique UUIDs), `hops` (1–2, default 1), free `predicate`, `time` | `ContextBundle/v1`: complete `testimony_context` and neighborhood-aware `fact_context` child envelopes, never a blended result list |
...
For `current` and `at`, supplying IDs invokes the default D97 recipe: P2 walks
...
and returned as facts, but they are never graph nodes. A missing P2 snapshot or
a current anchor absent from P2 is a `boundary`. A stale P2 neighbor is omitted
and counted in `dropped_by_hydration`, so current anchor facts can still return.
Caps are reported in `truncation`.
...
keep their explicit anchors without P2 expansion because the graph primitive is
point-in-time; the absent P2 freshness stamp makes that limitation visible.
