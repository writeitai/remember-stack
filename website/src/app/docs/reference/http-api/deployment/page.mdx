export const metadata = {
  ...
  description: "Check which code, models and tools a deployment is serving, probe its health, and read its cost export.",
};
...
| `tools` | Each MCP memory tool this deployment serves, by tool name, with its tool version. A tool appears only when the route it calls is served: the seven query tools (`query_sql`, `explain_sql`, `describe_query_space`, `search_query_space`, `list_saved_queries`, `describe_saved_query`, `run_saved_query`) only when the open query space is enabled, `delete_document` only when document deletion is, and `search_documents`
only when document search is. A tool's version rises whenever a call it accepts would fail, or mean something else, on a deployment serving the previous version. |
