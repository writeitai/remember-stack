...
`modified_from`/`modified_to`, `language`, `thread_ref` and `doc_ids`. Only
evidence from matching documents is returned:
...
- **claims** match when at least one place the claim occurs is in a matching
  ...
  else. The returned `chunk_id`, `char_start`/`char_end` and
  `evidence_spans` all describe that occurrence, in that version's text, not
  the chunk the claim was first extracted from; `source_span` is the claim's
  text, which is the same in every version that repeats it.
...
| `documents` | [`DocumentSearchFilters`](/docs/reference/result-types#documentsearchfilters) | no | | Only evidence from matching documents; see [Limiting search to certain documents](#limiting-search-to-certain-documents). |
