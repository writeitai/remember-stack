...
`status_code`, `detail` and, on the `/query/*` routes and on a `429`, `code`
(plus `retry_after` on a `429`). A network failure has `status_code` `0`.
...
After the credential check and before the route runs, every request except
`GET /healthz` is counted against two sets of limits: one for its credential
and one for the whole deployment.
...
effective limits are that many times larger.
