...
[openapi.json](https://github.com/writeitai/remember-stack/releases/download/v0.17.3/openapi.json).
...
remember.dev issues these signed keys:

| Credential | Where it comes from | Scope it carries |
|---|---|---|
| API key, `rmb_…` | Console (Settings → API keys) or `remember login`. Covers one, several or all projects of an organisation. | `write` with `memory:write`, `read` with only `memory:read` |
| Hosted MCP call credential | Signed by remember.dev for one tool call on `https://remember.dev/mcp`; lives 5 minutes. | `read`, or `ingest` for the `ingest` tool, or `write` for `delete_document` |
...
the settings below.<Cloud> Cloud deployments have them set.</Cloud> Each limit
...
after the credential check and before<Cloud> any spend hold or</Cloud> routing,
