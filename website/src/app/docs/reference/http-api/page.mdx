...
| Signed key, `kind: key` | A long-lived key a person created at a key issuer (remember.dev keys start with `rmb_`). It can cover several projects. | From its permissions |
