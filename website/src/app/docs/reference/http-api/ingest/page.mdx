...
`versions: current` a document is judged by its current version, or, while
it has none yet because its first upload is still processing, by its newest
version. With `versions: all` any version may match, and the result is the
...
passage. The two are combined by rank, and `matched_by` says which matched.
...
Without `query`, results are every document the filters match, newest
declared creation date first, documents without a date last, then by
`doc_id`. Pass `cursor` back for the next page. The first page fixes an
instant, returned as `as_of`, and every later page judges documents as they
were then, so documents added while you page do not shift the pages. Start a
new search to see them.
