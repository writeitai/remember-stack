...
early on a `failed` stage. See [Wait until a document is queryable](/docs/guides/wait-for-readiness).
...
Processing that was still running for the document finishes, but what it
produces is never visible, and it is retired as soon as that version reaches
the `reconcile` stage.
...
A second `DELETE` of the same document answers `404` `document_not_found`,
the same as an id the deployment never held. The one exception: if an
earlier call stopped part-way (the document was hidden, but its facts were
not yet updated), the next call finishes the job and answers `200` with what
it finished. Retrying after a timeout is therefore safe.
