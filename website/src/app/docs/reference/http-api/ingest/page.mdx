...
`200` means the version is stored. With `"parked": null` its processing is
queued; it is not yet queryable, so poll [`POST /readiness`](#post-readiness)
with the `version_id`. With `"parked": "no_route"` the deployment has no
converter for the `mime`: the bytes are kept, but nothing reads them until an
operator adds a route and releases the parked work. Do not wait on readiness
