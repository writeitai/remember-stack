export const metadata = {
  ...
  description: "Every HTTP status, error code and status value a RememberStack deployment and the remember client return, what each means, and what to do about it.",
};
...
deployment, the codes SQL queries report,<Cloud> spend refusals on remember.dev,</Cloud>
the structured errors the MCP tools return,<Cloud> `remember login` failures, the
remember.dev account API's error shape,</Cloud> and the status values that appear in
results.
...
| `401` | `perimeter authentication failed` | The credential is not accepted: wrong secret, bad signature, expired, revoked, or unknown signing key. | Get a fresh token.<Cloud> On remember.dev, run `remember login` again or create a token in the console.</Cloud> |
...
| `403` | `credential may not perform this operation` | The credential's scope does not cover the route (for example a `read` credential on `POST /ingest` or `DELETE /documents/{doc_id}`, or an `ingest` credential on `GET /deployment`). | Use a `write` credential. See [Scopes](/docs/reference/http-api#scopes). |
...
| `413` | `body_too_large` | The ingest body is over the cap<Cloud> (100,000,000 bytes on remember.dev)</Cloud>. | Split the document. |
...
| `429` | `{"code": "rate_limited", …}` | The credential or the deployment has used its request rate ([admission limits](/docs/reference/http-api#admission-limits)). | Wait the `Retry-After` seconds, then retry. |
...
remember.dev meters processed text and adds these:

| Status | `detail` | Meaning | What to do |
|---|---|---|---|
| `409` | `rate_class_unavailable` | Text metering: the file is a binary format. | Send plain text or Markdown. |
...
| `unauthorized` | 401 | no | Refresh or replace REMEMBER_API_KEY; re-mint if the token was revoked. |
...
| `transport_error` | 0 | yes | Retry with back-off; check REMEMBER_API_URL, credentials, and network reachability. |
...
`remember login` runs the OAuth device authorization grant (RFC 8628)
against the endpoints in the issuer's metadata. It polls the token endpoint
no faster than the interval the server gives, and never waits more than 30
...
| `429` or `503` | Keeps polling, no faster than `Retry-After`. |
...
| any other error | Stops with `token polling failed with HTTP <status> (<error>)`. |
| code expires while polling | Stops: `the device code expired before approval`. |
| metadata unavailable | Stops, naming the metadata URL. |
| a key issued by another issuer | Stops, and revokes that key. |
| redirect to another host | Stops: `refusing a cross-origin redirect`. |

Every failure exits with status 1 and leaves the stored key unchanged;
Ctrl-C exits with status 130. See [Tokens and sign-in](/docs/cloud/tokens-and-sign-in).
...
`client.account` raises `remember.MemoryApiError` with the status and the
message, and `remember.RateLimited` on a `429`.
...
returns this decision with its `reason_code`.
