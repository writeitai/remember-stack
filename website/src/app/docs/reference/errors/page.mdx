...
| `403` | `credential may not perform this operation` | The credential's scope does not cover the route (for example a `read` credential on `POST /operations/{name}` or `GET /deployment`). | Use a `write` credential. See [Scopes](/docs/reference/http-api#scopes). |
