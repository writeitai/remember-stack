...
known code at its expected status, and on a `429`, which also sets
`retry_after`). `wait_for_readiness` raises
...
| `401` | `perimeter authentication failed` | The credential is not accepted: wrong secret, bad signature, expired, revoked, or unknown signing key. | Get a fresh token.<Cloud> On remember.dev, create one in the console or run `remember login`.</Cloud> |
