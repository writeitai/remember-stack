...
| `429` | `{"code": "rate_limited", …}` | The credential or the deployment has used its request rate ([admission limits](/docs/reference/http-api#admission-limits), off unless configured). | Wait the `Retry-After` seconds, then retry. |
...
| `code` | `status_code` | `retryable` | `agent_action` |
|---|---|---|---|
| `spend_safety` | the HTTP status | no | Surface the spend/reservation refusal to the user/operator; do not busy-retry. Adjust budgets or wait for a new reservation. |
...
Ctrl-C exits with status 130. See [API keys and sign-in](/docs/cloud/tokens-and-sign-in).

## remember.dev account API errors

The remember.dev account API (organisations, projects, billing, API keys) uses
one error envelope:
