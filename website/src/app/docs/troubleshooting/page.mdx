...
| Empty / wrong answers | Grain misuse · readiness · projections · recipe args (`name`/`query`) |
...
`GET /recipes` must list exactly three assured ops: `resolve_entity`, `question_context`, `current_context`.
...
| Using `current_context` vs `question_context` | Wrong grain for the question |
| Recipe body uses `query` / `name` | Wrong args → 422 or empty intent |
...
| --- | --- | --- |
| `question_context` “wrong CFO” after roster edit | Evidence grain / old testimony may still exist as history | `current_context` + currency rules |
