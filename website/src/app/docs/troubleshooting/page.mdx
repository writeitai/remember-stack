...
Smoke profile routes **Markdown** and **plain text**. A file whose MIME type has no registered converter is **still ingested** — the original stays in the raw bucket and the document appears in the corpus filesystem with a `(not converted)` stub carrying its `raw_uri`, so an agent can still mount and read it. Only its conversion is **parked**: `processing_state` shows `status=pending`, `defer_reason=no_route`, `attempts=0`, and nothing reaches the dead-letter queue.
...
Parked work converts on the next worker pass; nothing needs re-uploading.
