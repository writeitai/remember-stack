...
skip to [Why RememberStack](/docs).
...
![Asked who leads the billing migration, search returns three passages that look equally relevant; a memory system returns one current fact with its date and source, and keeps the older statements as history.](/docs/diagrams/search-vs-memory.svg)
...
It is open source<Cloud>, and remember.dev runs it for you</Cloud>. Continue with
[Why RememberStack](/docs), or take the
[five-minute tour](/docs/start/how-it-works) to see what happens to a document once
you send it.
