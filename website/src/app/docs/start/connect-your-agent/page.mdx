...
opens a browser, you sign in, and you choose the projects and permissions
it gets. No key goes into the agent's configuration.
...
{
  "mcpServers": {
    "remember": { "url": "https://remember.dev/mcp" }
  }
}
```

Or let the `remember` package write the configuration for every agent it
finds:
...
Cursor, Claude Code and Codex get the endpoint URL and sign in through the
browser. Claude Desktop and Antigravity, which only start local programs,
get `remember mcp` as a bridge to the endpoint, using the key from
`remember login` (the command runs it first if you are not signed in). On a
machine without a browser, such as CI, add `--headless`: the agents then
send the key from `REMEMBER_API_KEY`. No key is written into any agent's
configuration. See [`remember setup`](/docs/reference/cli#remember-setup).

Setup for other clients, signing in with an API key instead, the account
tools, and how to disconnect are on [Hosted MCP](/docs/cloud/hosted-mcp).
...
| `search_documents` | Find a file by its name, title, path, authors, dates or text. |
| `adjacent_chunks` | Read neighbouring passages preceding and succeeding a target chunk. |
...
The hosted remember.dev server exposes the tools of the engine version
remember.dev runs, which does not have `search_documents` or
`adjacent_chunks` yet, plus account tools for projects, keys and balance.
Every tool there takes a `project` argument. See
