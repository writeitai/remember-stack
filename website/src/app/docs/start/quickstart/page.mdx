...
2. Open **Settings → API keys** and create a key. The defaults (your
   projects; read, write and account read) are what this page needs.
3. Copy the key. It starts with `rmb_` and is shown once.

Any member can create a key for the projects they can use. Instead of
copying a key, you can also run `remember login` after installing the
client (step 2); it stores a key on this machine. See
...
[Requirements](/docs/self-hosting/requirements#docker-and-compose).
...
[v0.17.3](https://github.com/writeitai/remember-stack/releases/tag/v0.17.3)
([on PyPI](https://pypi.org/project/remember/0.17.3/)).
...
The Python client and the `remember` CLI both read it, and ask
remember.dev for your default project's address. Set `REMEMBER_PROJECT`
to use another project the key covers. After `remember login` you need
neither variable.
