...
Remember connects seamlessly to modern AI coding agents via the **Model Context Protocol (MCP)**. Other surfaces (App, Python, CLI, HTTP): [Choose your path](/docs/getting-started/choose-your-path).
...
| :--- | :--- | :--- |
| `facts_context` | Fact Recall | Retrieves current or historical adjudicated facts with supporting evidence under a token budget. |
| `combined_context` | Comprehensive | Returns a dual-grain `ContextBundle` containing both raw testimony (claims) and adjudicated truth (facts). |
| `claims_and_sources_context` | Evidence Search | High-recall semantic retrieval over raw claims and chunk source text. |
