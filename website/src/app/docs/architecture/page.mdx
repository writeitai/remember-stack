...
indexes are derived state beside the authority rows; the graph carries a
filtered snapshot. Neither can become independent truth.
...
| --- | --- | --- |
| **P1** | PostgreSQL 18 (`pgvector` + `pg_textsearch`) | Vector + BM25 over chunks and claims; semantic search over fact labels and entities |
