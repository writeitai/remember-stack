...
Fact sheets show the chosen world window and its precision, alongside a saved
evaluation time. Completed relations remain in dated history. "Current" in a
saved page means current at that recorded evaluation time; consult fact retrieval
for what holds now. Unknown dates are stated explicitly.

Changing a fact window changes the K input fingerprint. The existing routing and
compile drivers compare those fingerprints and refresh affected pages. A custom
library composition must supply its compile context and driver; the stock E3/P1
worker loop does not run a K compiler.
...
Scopes share one entity space and one fact store. They own **markdown**, not separate truth databases.
...
Assured ops that return `grain: compiled` are orientation. For present-tense belief use `facts_context` / `facts_current`.
...
| [Architecture](/docs/architecture) | Durable store and what lives where |
