...
**Core principles and stances are authored** (D73). There is no separate machine-promoted “belief tier” that upgrades evidence into principles.
...
| Scope pattern | Role |
| --- | --- |
| **K1 — general knowledge** | Default scope: entity pages, topic/community pages, source digests, root index |
...
1. Facts/claims arrive **pre-labeled** (entities, predicates, communities) from Plane E.  
...
Assured ops that return `grain: compiled` are orientation. For present-tense belief use `current_context` / `facts_current`.
...
K is never a structural input to P rebuilds
```
...
| Repo design | `plan/designs/k_layers_design.md` (binding full mechanism) |
