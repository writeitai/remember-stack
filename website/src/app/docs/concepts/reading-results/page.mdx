...
candidate means ambiguity. See [Entities](/docs/concepts/entities).
...
| `validity` | `valid_from`, `valid_until` (exclusive), `valid_precision`, `ingested_at`, `invalidated_at`. See [Time](/docs/concepts/time). |
| `temporal_match` | `confirmed` or `possible`. |
| `contradiction_group` | The contradiction group's ID, or `null`. |
| `contradiction` | The other sides of the contradiction, inline. See [Contradictions](/docs/concepts/contradictions). |
| `support` | `current`, or `withdrawn` when its only support was lost to a processing change. See [Facts](/docs/concepts/facts#support-withdrawn). |
...
See [Claims](/docs/concepts/claims) and [Evidence](/docs/concepts/evidence).
...
[Evidence](/docs/concepts/evidence#why-do-we-believe-this).
...
- [Result types](/docs/reference/result-types): the complete schemas.
- [Handle unknowns and ambiguity](/docs/guides/unknowns-and-ambiguity).
