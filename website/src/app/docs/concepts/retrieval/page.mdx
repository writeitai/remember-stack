...
remember.dev also runs a hosted MCP server at `https://remember.dev/mcp`.
It has the same tools as `remember mcp` for the engine version remember.dev
runs, with a `project` argument on each, and adds account tools. See
