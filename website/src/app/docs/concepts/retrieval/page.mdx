...
|---|---|---|
| Search claims | `search_claims(query=, k=10, channel="semantic", documents=None)` | Claims, from one search channel (`semantic` or `bm25`), `k` up to 400; `documents` limits them to matching documents. |
...
search then considers only evidence from matching documents, before choosing
its best `k` results. See
