...
![A document is stored, converted, structured, chunked, turned into claims and checked against the source; then names are resolved to entities, claims become facts, facts are adjudicated and reconciled, and everything is indexed until the document is queryable.](/docs/diagrams/pipeline.svg)
...
![The pipeline stages and the order they run in.](/docs/diagrams/pipeline-stages.svg)
...
The details of each step are in [Claims](/docs/concepts/claims), [Facts](/docs/concepts/facts),
[Entities](/docs/concepts/entities) and [Updating a source](/docs/concepts/updating-sources).
...
[assured operations](/docs/concepts/retrieval) and retrieval primitives run on
...
[Operating the pipeline](/docs/self-hosting/operating) for how to inspect and
replay it.
...
| `p3` | A filesystem snapshot of the corpus has been published after your versions finished. | The read-only filesystem views. Self-hosted only; see [Filesystem views](/docs/self-hosting/filesystem-views). |
...
`dead_letter`. See [Wait until a document is queryable](/docs/guides/wait-for-readiness).
...
- [Ingest, readiness, documents](/docs/reference/http-api/ingest):
  `POST /readiness`.
- [Operating the pipeline](/docs/self-hosting/operating) (self-hosted).
