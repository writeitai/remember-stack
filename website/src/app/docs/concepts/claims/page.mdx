...
of the bare reference, but only when the document itself is what the claim
is about. It uses the version's title, or its file name when the version
declares no title:
...
listed in `added_context` like any other addition. Claims about anything
else never get the name added, including ordinary facts stated in an overview
of the document ("Q3 revenue was €4.2M" stays as it is): every claim already
...
- it appears exactly once in the claim as a whole name ("Report" inside
  "Reporting" does not count)
- it does not appear as a whole in any source text the claim cites
...
reuses the claim.
The recorded name does not become an [entity](/docs/concepts/entities).
The exception: when the claim has several references with that exact text,
the engine cannot tell which one is the document, so it resolves them all.

A new version with a different file name is extracted again, because the file
name is one of the inputs that decides whether a new version can reuse earlier
claims. Sending the same bytes under a new name creates no new version; the
new name is recorded for the existing version, and its claims stay as they
are. Claims are immutable, so a later rename never rewrites claims that name
the old file.
