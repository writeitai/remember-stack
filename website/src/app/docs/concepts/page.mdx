...
| **Query-time `resolve_entity`** | Exact alias, then trigram and phonetic blocking, then embedding search. Ranked candidates on ambiguity — never a silent single guess or an LLM adjudication |
...
Merges follow survivor chains with redirects disclosed. Query-time resolve recalls inflections and spelling variants through trigram blocking, then phonetic blocking, then profile embedding. Evidence search remains the path for testimony that never became an alias. There is no deployment-wide exact-name auto-accept switch in the shipped cascade.
...
- [Architecture](/docs/architecture) — durable store, what lives where, write vs read
