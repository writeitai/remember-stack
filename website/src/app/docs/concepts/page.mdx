...
An entity profile is orientation, not identity or a type. It is a disposable
projection of current supported observations and relations. Write-time T3
compares the mention name plus its claim context with the candidate name plus
profile and salient facts; it never compares two name-only vectors. An entity
with no evidence has no T3 vector and reaches gated T4 instead. T4 receives the
candidate profile and the current salient statements explicitly.

At write time, an exact name is only a T0 candidate signal. It never silently
accepts or merges an entity. One candidate with a current profile may be
accepted by T3. Empty or conflicting evidence, and every multiple-candidate
case, reaches tri-state T4: `same`, positively supported `different`, or
`insufficient_evidence`. Only `different` creates a durable cannot-link. If
evidence is insufficient or the bounded candidate pass is truncated, ingestion
may mint an active provisional fragment; its decision is explicit and later
profile evidence can repair it instead of being blocked by a false exclusion.

Profile refreshes are evidence-change debounced. If evidence is withdrawn, the
cached summary and vector rebuild or clear. Hard-forgetting one source of a
shared entity rebuilds from the surviving sources, so forgotten phrases do not
remain in the summary or semantic vector.

A successful evidence-backed profile publication nominates that entity's alias
neighborhood for bounded convergence. Automatic merging remains off until its
profile-space threshold is calibrated; equivalent open review proposals are
deduplicated, and every accepted merge remains a reversible redirect.

Merges follow survivor chains with redirects disclosed. For nickname/misspelling recall at read time, use evidence search (`testimony_context`) rather than assuming fuzzy resolve. There is no exact-name auto-accept switch in the shipped cascade.
