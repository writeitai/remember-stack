...
Merges follow survivor chains with redirects disclosed. For nickname/misspelling recall at read time, use evidence search (`testimony_context`) rather than assuming fuzzy resolve. There is no deployment-wide exact-name auto-accept switch in the shipped cascade.
