...
with no evidence has no T3 vector and reaches T4 instead. T4 receives every
candidate in the bounded snapshot with aliases, current profile description,
salient statements, and its T3 score or gate.
...
case, reaches one binary, match-biased T4 call on the configured simple model.
