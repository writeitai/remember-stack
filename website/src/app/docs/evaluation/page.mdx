...
Use these when changing the store, retrieval, or lifecycle behavior: failing a design eval is a product regression even if a demo still “looks fine.”
...
- Current protocol is `RS-LoCoMo-Full-v38`, versioned and fingerprinted in the adapter README.
...
### Development retrieval-access ablations

The D124 answer-only runner can reuse one processed Full-v38 sample across four
OSS-only access profiles: native Codex with a locally copied P3 directory,
native Codex with P3 plus the read-only RememberStack MCP server, an OpenRouter
Luna agent with only that MCP surface, and the same provider-neutral loop with
three bounded P3 list/search/read tools. It does not require UMC, RememberFS or
FUSE and it never changes the source run.

Treat only the within-runtime pairs as causal comparisons. Native Codex uses an
instruction-and-audit policy rather than a hard security sandbox, and its score
is not comparable until an operator has reviewed its action log and marked it
`clean`. Full commands and recovery rules are in the adapter README.
...
Full-v30 also prevents an identity-only lookup from becoming a terminal `Unknown`:
