...
class TokenHostSettings(BaseSettings):
    """CLI token-host and config-dir settings (``REMEMBER_`` or ``REMEMBERSTACK_`` prefix)."""

    model_config = SettingsConfigDict(env_prefix="REMEMBERSTACK_", extra="ignore")

    token_host: str | None = Field(
        default=None,
    ...
    config_dir: Path | None = Field(
        default=None,
        validation_alias=AliasChoices(
            "REMEMBER_CONFIG_DIR", "REMEMBERSTACK_CONFIG_DIR", "config_dir"
        ),
    )
...
class CliClientEnv(BaseSettings):
    ...
    internal_ops: bool = Field(
        default=False,
        validation_alias=AliasChoices(
            "REMEMBERSTACK_INTERNAL_OPS", "REMEMBER_INTERNAL_OPS", "internal_ops"
        ),
    )


class ControlPlaneCredentials(BaseModel):
    """Control plane user session credentials (D108)."""
...
class ProjectCredentials(BaseModel):
    """Per-tenant data plane configuration and scoped token (D108)."""

    model_config = ConfigDict(extra="ignore", frozen=True, hide_input_in_errors=True)

    name: str
    data_plane_url: str
    data_plane_token: SecretStr
...
class CredentialFile(BaseModel):
    """Version-1 credential document (D92/D108). Unknown version refuses to read.
    ...
    # D108 structured credential fields
    control_plane: ControlPlaneCredentials | None = None
    active_project_id: str | None = None
    projects: dict[str, ProjectCredentials] | None = None
    ...
    @property
    def active_data_plane_url(self) -> str | None:
        if (
            self.active_project_id
            and self.projects
            and self.active_project_id in self.projects
        ):
            return self.projects[self.active_project_id].data_plane_url
        return self.api_url

    @property
    def active_data_plane_token(self) -> SecretStr | None:
        if (
            self.active_project_id
            and self.projects
            and self.active_project_id in self.projects
        ):
            return self.projects[self.active_project_id].data_plane_token
        return self.access_token

    def get_active_project(self) -> ProjectCredentials | None:
        if (
            self.active_project_id
            and self.projects
            and self.active_project_id in self.projects
    ...
    def get_active_project_token(self) -> str | None:
        tok = self.active_data_plane_token
...
def credentials_dir(*, settings: TokenHostSettings | None = None) -> Path:
    ...
    Checks for an existing credentials.json in ~/.config/remember first,
    falling back to ~/.config/rememberstack if present, preventing an
    empty ~/.config/remember directory from shadowing legacy credentials.
    """
    resolved = (
    ...
    remember_dir = (
        (xdg / "remember")
        if xdg is not None
        else (Path.home() / ".config" / "remember")
    )
    legacy_dir = (
        (xdg / "rememberstack")
        if xdg is not None
        else (Path.home() / ".config" / "rememberstack")
    )
    if (remember_dir / "credentials.json").is_file():
        return remember_dir
    if (legacy_dir / "credentials.json").is_file():
        return legacy_dir
    return remember_dir
...
def write_credentials(
    *, credential: CredentialFile, settings: TokenHostSettings | None = None
    ...
    if credential.control_plane is None:
        dumped.pop("control_plane", None)
    else:
        if credential.control_plane.access_token is not None:
            dumped["control_plane"]["access_token"] = (
                credential.control_plane.access_token.get_secret_value()
            )
    if credential.active_project_id is None:
        dumped.pop("active_project_id", None)
    if credential.projects is None:
        dumped.pop("projects", None)
    else:
        for proj_id, proj in credential.projects.items():
            if proj.data_plane_token is not None:
                dumped["projects"][proj_id]["data_plane_token"] = (
                    proj.data_plane_token.get_secret_value()
                )
