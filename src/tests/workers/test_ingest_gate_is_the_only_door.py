"""D117 holds only while E0 is the single door into a document version.
...
- a new caller reaching `DocumentCatalog.record_upload` directly, creating a
  version whose convert work was scheduled without consulting the route table;
- a new module writing `document_versions` itself, going around the catalog;
- `routable_mimes` regaining a default, letting a composition skip the
...
def test_the_route_table_cannot_be_omitted_by_a_composition() -> None:
    ...
    An earlier draft defaulted it to None. That made D117 as strong as every
