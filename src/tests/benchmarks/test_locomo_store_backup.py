...
def _run_json(path: Path) -> None:
    ...
    (path / "run.json").write_text(
        json.dumps(
            {
                "protocol_name": "RS-LoCoMo-Full-v20",
...
def test_shard_runner_guards_wipe_and_backs_up_before_scoring() -> None:
    ...
    assert "REMEMBERSTACK_E2_EXTRACT_MODEL=z-ai/glm-5.3-flash" in script
    assert "REMEMBERSTACK_OBS_FRONTIER_MODEL=z-ai/glm-5.3-flash" in script
