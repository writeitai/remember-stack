...
@pytest.mark.parametrize(
    ...
    (
        ("full-v33", "openai/gpt-5.6-luna", "openai/gpt-5.6-luna", "none", 0.0, 0, 2),
        (
...
def test_ingest_refuses_document_binding_generation_drift_before_upload(
    ...
    """Full-v33 cannot silently process with document-local T0 disabled."""
    _patch_prepared_inputs(monkeypatch=monkeypatch)
    run_dir = tmp_path / "run"
    prepare_run(dataset_path=tmp_path / "synthetic.json", tier="smoke", output=run_dir)
    uploads = 0
...
def test_single_run_summary_json_is_unchanged(
    ...
    assert serialized == (
        '{"protocol_name":"RS-LoCoMo-Full-v33","protocol_fingerprint":'
        '"ac7ab489948c6512e80af8a63d3d439228ef7a25fcd41ddedc13cd9c674b4e89",'
...
def test_prepared_protocol_pins_current_surface_and_luna(
    ...
    assert prepared.protocol_name == "RS-LoCoMo-Full-v33"
...
def test_run_protocol_resolves_the_prepared_variant(tmp_path: Path) -> None:
    ...
    protocol = PROTOCOL_REGISTRY["full-v33-gemma-vertex"]
