...
def test_protocol_is_v36_and_answer_prompt_has_reasoning_and_loop_guards() -> None:
    """The current identity, bounded inference, and loop discipline are locked."""
    assert PROTOCOL_NAME == "RS-LoCoMo-Full-v36"
    assert DEFAULT_PROTOCOL_KEY == "full-v36"
...
def test_typed_protocol_registry_pins_answer_agent_identity_and_effort() -> None:
    assert tuple(PROTOCOL_REGISTRY) == (
        "full-v36",
        "full-v36-gemma-vertex",
        "full-v36-codex-subscription",
    )
    protocol = PROTOCOL_REGISTRY["full-v36"]

    assert protocol.name == "RS-LoCoMo-Full-v36"
...
def test_gemma_vertex_variant_swaps_only_the_answer_agent() -> None:
    """The variant is a reader-only provider swap over identical v36 pins, so its
    scores are an answer-agent comparison rather than a new benchmark identity."""
    base = PROTOCOL_REGISTRY["full-v36"]
    variant = PROTOCOL_REGISTRY["full-v36-gemma-vertex"]

    assert variant.name == "RS-LoCoMo-Full-v36-GemmaVertex"
...
def test_cli_composes_only_the_vertex_answer_seat(
    ...
    _write_run_json(run_dir=tmp_path, protocol_key="full-v36-gemma-vertex")
...
def test_cli_keeps_plain_openrouter_for_the_default_protocol(
    ...
    _write_run_json(run_dir=tmp_path, protocol_key="full-v36")
    monkeypatch.setenv("REMEMBERSTACK_OPENROUTER_API_KEY", "test-key")
    monkeypatch.delenv("REMEMBERSTACK_VERTEX_PROJECT_ID", raising=False)

    def refuse(**_values: object) -> object:  # pragma: no cover
        raise AssertionError("Vertex must not be composed for full-v36")
...
def test_codex_subscription_variant_pins_both_generation_seats() -> None:
    ...
    base = PROTOCOL_REGISTRY["full-v36"]
    variant = PROTOCOL_REGISTRY["full-v36-codex-subscription"]

    assert variant.name == "RS-LoCoMo-Full-v36-CodexSubscription"
...
def test_cli_codex_answer_and_judge_need_no_openrouter_key(
    ...
    _write_run_json(run_dir=tmp_path, protocol_key="full-v36-codex-subscription")
