...
Observation applications incorporate accepted D113 (b776b3e5).
...
_OBSERVATION_DOWNGRADE_DDL = r"""
DROP TABLE public.temporal_checkpoint_observation_support, public.observation_application_adjudications, public.observation_applications, public.observation_apply_batches CASCADE;
ALTER TABLE public.normalize_observation_staging
  DROP COLUMN membership_id CASCADE, DROP COLUMN adjudicator_version CASCADE,
  DROP COLUMN flush_version CASCADE, DROP COLUMN assertion_id CASCADE, DROP COLUMN applied_at,
  ADD PRIMARY KEY (deployment_id, version_id, claim_id, subject_entity_id, statement, normalizer_version);
ALTER TABLE public.obs_flush_entity_units
  DROP COLUMN adjudicator_version CASCADE, DROP COLUMN flush_version CASCADE,
  DROP CONSTRAINT obs_flush_entity_units_deployment_id_unit_id_key,
  ADD UNIQUE (deployment_id, version_id, normalizer_version, subject_entity_id);
ALTER TABLE public.obs_flush_version_state
  DROP COLUMN adjudicator_version CASCADE, DROP COLUMN flush_version CASCADE,
  DROP COLUMN lane, DROP COLUMN expected_units,
  ADD PRIMARY KEY (deployment_id, version_id, normalizer_version);
ALTER TABLE public.observation_adjudications
  DROP COLUMN triggering_assertion_id,
  DROP CONSTRAINT observation_adjudications_deployment_id_adjudication_id_key;
ALTER TABLE public.observation_evidence DROP COLUMN legacy_support;
"""
...
def upgrade() -> None:
    ...
    for statement in _split_sql(
        sql=TEMPORAL_EXPAND_DDL + _TABLE_COMMENTS_DDL + OBSERVATION_APPLICATION_DDL
    ):
