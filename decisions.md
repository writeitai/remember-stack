...
4. **Binary Ownership & Platform CLI**: The `remember` executable is owned exclusively by the `remember` package. It unifies:
   ...
   - **Resilient Agent Bootstrapper (`remember setup`)**: When bootstrapping agent harnesses via `uvx`, generates durable launch commands using resolved launcher binaries (`shutil.which("uvx")` / `shutil.which("remember")` or explicit tool paths) so configured MCP servers launch reliably across environment reboots and GUI editor spawns without PATH inheritance issues.
