...
3. **Full Cloud Data-Plane Parity (`open_query`)**: The open query space is fully enabled on Remember Cloud. Because every cloud project runs in a dedicated private pod and database instance (physical pod isolation, zero shared multi-tenant tables), and queries are AST-validated with PostgreSQL's parser (`pglast`), deny-by-default read-only transaction sandboxed (`QuerySandboxExecutor`), and metered under request spend safety, complete feature parity with self-hosted instances is guaranteed.
