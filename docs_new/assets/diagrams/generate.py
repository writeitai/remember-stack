...
def head(*, title: str, desc: str, width: int = 1200, height: int = 640, steps: int = 10, extra_css: str = "") -> str:
...
'''
    claims = [
        ("Kickoff notes · 15 Jan", "“Ravi has been on the billing", "migration since yesterday.”", 160),
...
'''
    s = head(
        ...
        height=600, steps=8, extra_css=cursor_css,
    )
    s += f'''  <text class="k" x="60" y="62">ONE FACT, THREE CLOCKS</text>
  <text class="h" x="60" y="92">Ravi works on the billing migration</text>
'''
    for m, d in zip(months, starts):
        s += f'  <text class="sm" x="{x(d) + 4:.0f}" y="136">{m}</text>\n  <line x1="{x(d):.0f}" y1="146" x2="{x(d):.0f}" y2="470" stroke="#18244f"/>\n'
    lanes = [("WORLD TIME", "When was it true?", 200), ("SAID-ON TIME", "When did a source say it?", 300), ("BELIEF TIME", "When did the memory learn it?", 400)]
...
'''
    s = head(
        ...
        height=560, steps=10, extra_css=css,
    )
    row1 = [("Store", "raw file kept"), ("Convert", "to Markdown"), ("Structure", "find sections"), ("Chunk", "cut passages"), ("Extract", "select, rewrite"), ("Ground", "check vs source")]
    row2 = [("Resolve", "names → entities"), ("Form facts", "relations, observations"), ("Adjudicate", "confirm, adjust, close"), ("Reconcile", "recount support"), ("Index", "search, graph"), ("Queryable", "readiness: ready")]
...
'''
    chans = [("Meaning", "vector search", 150), ("Keywords", "BM25", 265), ("Graph", "neighbours of entities", 380)]
    for i, (n, sub, y) in enumerate(chans):
        s += f'''  <g class="s2">
...
def provenance() -> str:
    ...
    boxes = [
        ("FACT", "Dana leads the billing", "migration · since 1 Jun", 60, "cardT", "sphereT"),
        ("CLAIM", "“Dana now leads the", "billing migration.”", 340, "card", "sphereC"),
        ("PASSAGE", "Retro notes, section", "“Team changes”", 620, "card", "sphereS"),
        ("DOCUMENT", "retro-2026-06-12.md", "version 1 · chars 412–455", 900, "card", "sphereS"),
    ]
...
if __name__ == "__main__":
    ...
    for name, fn in [("claims-and-facts", claims_and_facts), ("three-clocks", three_clocks), ("pipeline", pipeline), ("read-path", read_path), ("provenance", provenance)]:
