...
| `REMEMBERSTACK_OPENROUTER_ZDR` | `false` | Restricts chat calls to zero-data-retention endpoints. Not passed by `compose.yaml`; set it through `engine.env` ([Configuration](configuration.md#what-compose-passes-through)). |
...
[cost export](observability.md#cost-export), and cap it per stage with
[spend budgets](operating.md#spend-budgets). The provider account's own
limit is the final monetary boundary.

remember.dev chooses the models and supplies the model key for each
project's deployment; see [What remember.dev serves](../cloud/compatibility.md).
