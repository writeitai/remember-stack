...
5. **Get the new images:**
    ...
    All app services share the image `pull api` fetches. `build postgres`
    rebuilds the PostgreSQL image in case `Dockerfile.postgres` changed; it
    reuses the cache when nothing did. On arm64, build the app image
    instead of pulling it: `docker compose build api`.
...
- With a new slug as well, `setup` creates a second deployment in the same
  ...
  secrets bound to the old id and signed tokens with the old id as
  audience are refused with `403`, and the cost export stops working
...
| `rememberstack_minio-data` | Yes: original files and derived artifacts |
...
Filesystem-view snapshots in MinIO can be rebuilt; everything else in the
...
docker compose stop
mkdir -p backup
for volume in postgres-data minio-data forget-manifests app-state; do
  docker run --rm \
...
same deployment id and the same database and MinIO credentials.
...
docker compose up -d --wait
docker compose exec -T -e REMEMBERSTACK_INTERNAL_OPS=1 api remember ops graph-catalog ensure
```

The last command checks that the graph definitions in the restored database
match this release ([Operating the pipeline](operating.md#the-graph-catalog)).
...
queries, and roles are not part of `pg_dump`. Copy the MinIO buckets
(`remember-raw`, `remember-artifacts`, `remember-corpusfs`) at the same
time with an S3 tool such as `mc mirror`. Restoring a logical backup means
...
[What remember.dev serves](../cloud/compatibility.md) says which version it
runs.
