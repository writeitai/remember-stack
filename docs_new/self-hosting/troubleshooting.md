...
| `setup` fails after moving to 0.17.0 | `docker compose logs setup` | [setup fails on upgrade to 0.17.0](#setup-fails-on-upgrade-to-0170) |
...
[Operating the pipeline](operating.md) describes every field of the report.
...
| `setup` refuses because the deployment id, slug or name differ | `.env` no longer matches what the first `setup` recorded. | Restore the original values. See [The deployment id is permanent](upgrades.md#the-deployment-id-is-permanent). |
...
| The shared secret and the bind disagree, or a signing key is refused | `REMEMBERSTACK_SELFHOST_API_BEARER_TOKEN` and `REMEMBERSTACK_SELFHOST_API_BEARER_BIND` describe different secrets, or one key in `REMEMBERSTACK_SELFHOST_API_SIGNING_KEYS` is invalid. | See [Authentication and scopes](authentication.md). |
| `worker-convert` names an unknown converter | A route in `REMEMBERSTACK_SELFHOST_CONVERSION_ROUTES` names a converter that does not exist, or needs a key that is not set. | See [File formats and converters](converters.md). |
| A migration error in `setup` that says the store already holds claims | You moved a deployment with data to 0.17.0. | See [setup fails on upgrade to 0.17.0](#setup-fails-on-upgrade-to-0170). |
...
The scope surprise that catches most people: **every assured operation
needs `write`**, although they only read. A `read` token gets `403` on
`POST /operations/facts_context` and on the other three. `GET /deployment`
needs `write` too. Give an agent that calls operations a `write` token, or
limit it to the search, graph and SQL routes a `read` token may call. The
full table is in [Authentication and scopes](authentication.md#scopes).
...
[Browser origins](authentication.md#browser-origins-cors).
...
| Every stage done, `p1` or `live_graph` not ready | A deployment-wide index or graph check is failing, not your document. | [Operating the pipeline](operating.md#the-graph-catalog) |
...
[File formats and converters](converters.md#add-routes), apply it with
...
- the Python client with `bytes` instead of a path, and no `mime=`:
  `application/octet-stream`;
- the MCP `ingest` tool with `content_base64` and no `mime`:
  `application/octet-stream`, whatever the `filename`;
- a file with no extension, or an extension the sending machine's MIME
  table does not know;
- a `curl` call with a type such as `text/x-markdown`. The match is exact.

Re-sending the same file with the right type does not help. RememberStack
stores identical bytes once, and the type recorded with the first upload
stays with those bytes. To release what is parked, add a route for the
type that was recorded, for example
...
For new uploads, pass the type: `remember ingest notes.md --mime text/markdown`,
or `mime="text/markdown"` in Python.
...
at the input itself. See [Operating the pipeline](operating.md#retries-and-dead-letters).
...
| Sign | Cause | Fix |
|---|---|---|
| A large `pending` count on one stage in `routes` | That stage is the bottleneck. | Add replicas: `docker compose up -d --scale worker-extract-claims=3`. See [Scaling](scaling.md#more-replicas-of-a-stage). |
| Log lines with `overloaded (429)` | OpenRouter or a provider behind it is rate-limiting you. More replicas make it worse. | Lower the worker rate, or set a provider order; see [Scaling](scaling.md) and [Models and providers](models.md#openrouter-routing-and-limits). |
...
| Every call slow | Reasoning models spend time and tokens thinking. | Lower `REMEMBERSTACK_OPENROUTER_REASONING_EFFORT_MAP` for the small models; see [Models and providers](models.md#openrouter-routing-and-limits). |
...
| `returned 400` or `returned 404`, with the provider's message about the model | A model id in `.env` does not exist, or the model does not support structured output. | Use an exact model id of a model with structured output; see [Models and providers](models.md#change-a-model). |
| `provider returned an embedding dimension that differs from the request` | The embedding model does not return 1,536 dimensions. | See [Changing the embedding model](models.md#changing-the-embedding-model). |
| `mistral ocr` | An OCR route's Mistral call failed, or Mistral rejected the file. | Check `REMEMBERSTACK_MISTRAL_OCR_API_KEY`; see [File formats and converters](converters.md#mistral_ocr). |
...
`docker compose up -d`. See [Spend budgets](operating.md#spend-budgets).
...
|---|---|---|
| "Who works on the billing migration?" leaves out Ravi, who worked on it until June. | `facts_context` defaults to the `current` time mode, which returns only facts true now. | Ask with `time={"mode": "history"}`, or `at` a date. See [Time](../concepts/time.md#asking-about-time). |
...
| The old June deadline still appears after the notes moved it to October. | `claims_and_sources_context` returns what sources said, including the earlier statement. Claims never change. | Use `facts_context` for what holds now; use claims to show who said what. See [Facts](../concepts/facts.md). |
| "Dana" returns facts about the wrong Dana, or nothing. | `resolve_entity` returns every entity that matches and never picks one for you. | Resolve first; ask which one, or pass the right `entity_ids`. See [Handle unknowns and ambiguity](../guides/unknowns-and-ambiguity.md#names-that-match-more-than-one-entity). |
| The agent says "Ravi has no tasks" after a `boundary` negative. | `boundary` means the lookup could not run as asked, not that nothing exists. | Follow `workaround`. Brief your agent as in [Handle unknowns and ambiguity](../guides/unknowns-and-ambiguity.md#brief-your-agent). |
...
| The answer picks one side of a disagreement. | Both facts stand, in one `contradiction_group`. | Report both sides with their sources. See [Contradictions](../concepts/contradictions.md). |
| `422` `invalid_parameter` "unknown argument(s)" from an operation. | Each operation accepts a fixed set of arguments. | See [Assured operations](../reference/assured-operations.md). |
...
|---|---|---|
| `relation_not_allowed` | A table outside `memory_v1`, such as the internal `processing_state`. | Use the [views](../reference/query-space.md#views). |
...
| `function_placement_not_allowed` | A query-space function used outside a top-level `FROM` item. | Follow the [placement rules](../reference/query-space.md#public-function-placement). |
...
Every code is in [Errors and status codes](../reference/errors.md#sql-query-codes).
...
[Hard-forget manifests](upgrades.md#hard-forget-manifests) and
[When a hard forget is in progress](operating.md#when-a-hard-forget-is-in-progress).

## setup fails on upgrade to 0.17.0

0.17.0 changes how facts carry their time windows, how claims point at
their evidence and how sources are referenced. Three of its migrations do
not convert data written by earlier releases. On a database that already
holds claims or chunks, `setup` stops with an error that ends in
"recreate the deployment and ingest its sources again", and the API and
workers do not start.

`docker compose logs setup` shows the message. To move to 0.17.0:

1. If you may want to go back, take a backup of the old volumes first, as
   described in [Back up](upgrades.md#back-up), while the old release is
   still checked out.
2. Remove the old data: `docker compose down -v`. This deletes the memory.
3. Start 0.17.0: `docker compose up -d --wait`.
...
Read them before you share them. See [Contributing](../project/contributing.md).

On remember.dev you do none of this: the pipeline is operated for you, and
you send the `version_id` or request details to [Support](../cloud/support.md).
