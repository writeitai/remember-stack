---
title: Scaling
description: The bounds a self-hosted RememberStack runs within — worker claim rates, replicas per stage, connection pools for retrieval and graph queries, and SQL query limits.
...
These are not passed by `compose.yaml`. Set them in `engine.env` for the
worker services ([Configuration](configuration.md#what-compose-passes-through)).
Lowering the rate is the way to stay under a provider's rate limit; the
...
`remember ops inspect` ([Operating the pipeline](operating.md#inspect-the-pipeline)).
...
[SQL queries](../guides/sql.md) run in a sandbox over the query space
...
listed in [Limits](../cloud/limits.md).
