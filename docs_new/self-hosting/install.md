...
that ship in the repository. Check [Requirements](requirements.md) first,
...
it names (`ghcr.io/writeitai/remember-stack:0.17.0`). Each GitHub release
...
!!! warning
    ...
    [Upgrades and migrations](upgrades.md#the-deployment-id-is-permanent).
...
| `worker-convert` | `worker --stage convert` | Turns an uploaded file into Markdown using the configured [converter](converters.md). |
...
| `meter-receipts` | `meter-receipts` | Profile `managed` only. Used by remember.dev for billing; do not start it. |
| `projections` | `project --plane p3` | Profile `operations` only. Builds a [filesystem view](filesystem-views.md) snapshot on demand. |

Each worker handles one stage and wakes when new work for that stage is
committed. [The pipeline and readiness](../concepts/pipeline.md) explains
...
[Wait until a document is queryable](../guides/wait-for-readiness.md) shows
...
!!! note
    ...
    request goes. With no URL in the environment, the CLI uses the
    credentials that `remember login` stored, which point at remember.dev
    if you have ever signed in there. Only with no stored credentials does
    it fall back to `http://127.0.0.1:8000`.
...
[Upgrades and migrations](upgrades.md#back-up).
...
limit the published ports; see [Authentication and scopes](authentication.md).

On remember.dev you create a project instead, and it gets its own deployment
and endpoint; see [Organisations, projects and members](../cloud/organisations-and-projects.md).
