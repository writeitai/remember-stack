...
It pulls together the other guides: [Ingest files](ingest-files.md), [Wait
until a document is queryable](wait-for-readiness.md), [Give an agent
context](agent-context.md), [Cite the source of an
answer](cite-sources.md) and [Handle unknowns and
...
def load_notes(client: remember.Client, folder: Path) -> None:
    ...
    for start in range(0, len(pending), 1000):
        client.wait_for_readiness(
            pending[start : start + 1000], timeout=3600, poll_interval=30
        )
    print("Ready.")
...
of the same document. `mime="text/markdown"` is set explicitly because the
MIME guess for `.md` is not reliable on every Python. `wait_for_readiness`
gets an explicit timeout, because its 30-second default is shorter than a
real run.
...
- **Ask about the past.** Pass `time={"mode": "history"}` or an `at`
  ...
  July?" ([Ask about the past](ask-about-the-past.md)).
...
- **Give it tools instead of a fixed call.** Run `remember mcp` and let an
  ...
  ([Connect your coding agent](../start/connect-your-agent.md)).
