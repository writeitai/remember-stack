...
description: The routes that add a document, report how far its processing has got, and list what the deployment holds.
applies_to: [remember.dev, self-hosted]
---

# Ingest, readiness and documents

Three routes cover the write side of memory. `POST /ingest` adds one file.
...
bytes again is a no-op, and different bytes are a different document.
...
conversion waits until a converter route for that type exists. See
[File formats and converters](../../self-hosting/converters.md).
...
`200` with an [`IngestedVersion`](../result-types.md#ingestedversion):
...
`200` means the version is stored and its processing is queued. It is not yet
queryable. Poll [`POST /readiness`](#post-readiness) with the `version_id`.
...
`filename`. It guesses `mime` from the file name and falls back to
`application/octet-stream`. `Client.ingest_file(path, ...)` is the same call.
...
`200` with a [`PipelineReadinessReport`](../result-types.md#pipelinereadinessreport).
...
If a stage is `failed` or `dead_letter`, stop polling: that version will not
become ready without intervention.
...
report = memory.wait_for_readiness([version.version_id], timeout=300, poll_interval=2)
```

`wait_for_readiness` polls every `poll_interval` seconds (default 0.5) and
raises `TimeoutError` after `timeout` seconds (default 30). It does not stop
early on a `failed` stage. See [Wait until a document is queryable](../../guides/wait-for-readiness.md).
...
`200` with a [`DocumentPage`](../result-types.md#documentpage):
...
The `remember` Python client has no method for this route yet. Call it over
HTTP.
