...
| Group | Page | Routes |
|---|---|---|
| Ingest, readiness, documents | [Ingest](ingest.md) | `POST /ingest`, `POST /readiness`, `GET /documents` |
| Assured operations | [Operations](operations.md) | `GET /operations`, `POST /operations/{name}` |
| Entities and facts | [Entities and facts](entities-and-facts.md) | `GET /resolve`, `GET /lookup/relations`, `GET /lookup/observations`, `GET /hydrate/relation/{relation_id}`, `GET /transcript/relation/{relation_id}` |
| Search | [Search](search.md) | `GET` and `POST /search/claims`, `GET` and `POST /search/chunks`, `GET /chunks/{chunk_id}/adjacent`, `POST /chunks/adjacent` |
| Graph | [Graph](graph.md) | `POST /graph/neighborhood`, `POST /graph/path`, `POST /graph/citation-path` |
| SQL queries | [SQL queries](query.md) | `POST /query/sql`, `POST /query/sql/explain`, `GET /query/space`, `GET /query/space/search`, `GET /query/saved`, `GET /query/saved/{namespace}/{name}`, `POST /query/saved/{namespace}/{name}/run` |
| Deployment | [Deployment](deployment.md) | `GET /deployment` |
...
[Errors and status codes](../errors.md) lists every status, code and what to
do about it.
...
[Spend caps and auto top-up](../../cloud/spend-controls.md).
...
version (a revert). See [Ingest](ingest.md#post-ingest).
...
See [Result types](../result-types.md).
...
| Route | Status |
|---|---|
| `GET /healthz` | Served by the self-hosted profile as the container's liveness probe. Returns `{"status": "ok"}` after a `SELECT 1` against PostgreSQL. No credential needed. Not in the OpenAPI document. See [Deployment](deployment.md#get-healthz). |
