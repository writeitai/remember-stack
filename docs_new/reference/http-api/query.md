...
in [Query space memory_v1](../query-space.md). This page covers the seven
...
described in [HTTP API conventions](index.md).
...
[`QueryResult/v1`](../result-types.md#queryresultv1) **in every outcome**,
...
HTTP error, is in [Errors and status codes](../errors.md#sql-query-codes).
...
[Query space memory_v1](../query-space.md#limits) is not reachable over HTTP.
...
see [Query space memory_v1](../query-space.md#shipped-saved-queries).
...
| Status | `detail` | Cause |
|---|---|---|
| `500` | `Internal Server Error` | `status` is not one of the six values. |
...
[Query space memory_v1](../query-space.md#shipped-saved-queries).
