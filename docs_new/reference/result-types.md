...
!!! warning
    ...
    routes (`valid_at`, `believed_at`) in UTC, with `Z` or `+00:00`. The
    engine echoes them into the result's `temporal_scope`, which accepts only
    UTC; a timestamp with another offset currently fails with `500` instead of
    being converted. The assured operations' `time` object converts any
    offset to UTC for you.
...
| `GET /lookup/relations`, `GET /lookup/observations` | `fact` | `facts`, `dropped_by_hydration`, `negative` |
...
| `reason` | string, nullable | Which cap applied, when known: `resolve_candidate_limit`, `result_budget`, `expansion_budget`, `frontier_budget`, `time_budget`, `depth_budget`. |
...
| `error_code` | string, nullable | One of the [SQL query codes](errors.md#sql-query-codes). |
...
| `reason` | string | `ready`, or why not (see [Ingest](http-api/ingest.md#post-readiness)). |
...
| `mutates` | boolean, nullable | Whether the operation changes memory. `null` on all four today. |
...
routes: [Graph](http-api/graph.md), [Ingest](http-api/ingest.md#post-readiness),
