...
described in [Time](time.md). It also anchors relative dates: when a
...
!!! warning
    ...
    check [readiness](pipeline.md#readiness), not the document status.
...
- [Claims](claims.md): what RememberStack extracts from each version.
- [Updating a source](updating-sources.md): what a new version means, in
  `snapshot` and `living` mode.
- [Ingest files](../guides/ingest-files.md) and
  [Keep a source up to date](../guides/keep-sources-current.md).
- [Ingest, readiness, documents](../reference/http-api/ingest.md) for every
  parameter and error.
