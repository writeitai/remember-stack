...
`entity_ids`. See [Entities](entities.md#resolving-a-name-at-query-time).
...
| `time` | `{"mode": "current"}` | `current`, `at`, `overlap`, `history`; see [Time](time.md#asking-about-time) |
...
[Give an agent context](../guides/agent-context.md).
...
| Lookup relations | `lookup_relations(subject_entity_id=, predicate=, object_entity_id=, valid_at=)` | Relations matching a pattern, current or at one instant. |
...
read (see [Time](time.md#belief-time-and-as-of)). A graph path is returned
...
See [Search](../reference/http-api/search.md),
[Graph](../reference/http-api/graph.md) and
[Resolve, lookup, hydrate, transcript](../reference/http-api/entities-and-facts.md).
...
with parameters. SQL queries work the same on remember.dev and self-hosted.
See [Explore memory with SQL](../guides/sql.md),
[Saved queries](../guides/saved-queries.md) and
...
[corroboration count](contradictions.md#corroboration). The two lists come
back in one envelope, kept apart: `evidence` for claims, `chunks` for
passages.

## Where to go next

- [Reading a result](reading-results.md): the envelope field by field.
- [Assured operations](../reference/assured-operations.md): full schemas.
- [Give an agent context](../guides/agent-context.md) and
  [Build a memory-backed agent](../guides/build-an-agent.md).
- [MCP tools](../reference/mcp.md): what the `remember` MCP server exposes.
