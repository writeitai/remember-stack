...
Every [claim](claims.md) carries the world time its source stated, if any,
as four fields:
...
[Documents, versions and sources](documents-and-sources.md#source_modified_at-becomes-the-said-on-time).
...
Each [fact](facts.md) has **one** chosen world-time window: `valid_from`,
...
before and after. See [Evidence](evidence.md#why-do-we-believe-this).
...
!!! note "Claim ends are inclusive; fact ends are exclusive"
...
fact's only support (see [Updating a source](updating-sources.md)). Facts
...
[Graph](../reference/http-api/graph.md).
...
for "nothing was said". See [Saved queries](../guides/saved-queries.md)
and [Ask about the past](../guides/ask-about-the-past.md).

## Where to go next

- [Ask about the past](../guides/ask-about-the-past.md): recipes for
  point-in-time and period questions.
- [Facts](facts.md): how windows are chosen and corrected.
- [Reading a result](reading-results.md): `validity`, `temporal_match` and
  `temporal_scope` field by field.
- [Assured operations](../reference/assured-operations.md): the `time`
  argument schema.
