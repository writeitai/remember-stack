...
- **Action needed.** The Python client, the CLI and the MCP server ship in
  ...
  `import rememberstack` to `import remember`. See the
  [migration note](../reference/python-sdk.md#migrating-from-the-rememberstack-package).
...
- New client class `remember.Client` (also `remember.RememberClient`) with an
  ...
  `ingest()`. `remember.CloudClient` reads a remember.dev organisation's
  deployment, balance, ledger and spend gate.
