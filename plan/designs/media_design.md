...
including `always_ocr_and_description.md` for the owner-approved dual-call simplification.
...
>   the **raw mount** (D51/D116) serves immutable originals read-only, on the navigation path
...
| **Image** (`image/*`) | **both lanes, always**: dedicated OCR and one independent vision-LLM description call | the description and a "Visible text (OCR)" section, clearly sectioned; region-grain descriptions permitted as sub-sections with image-region locators | the **agent rendition** (§4a) plus any crops/thumbnails |

Notes an implementer needs:

- **Every supported image runs dedicated OCR and a vision LLM description call.**
  These are independent lanes receiving the same source image, executed concurrently
  within deployment concurrency limits. There is no image classifier, exclusive route,
  conditional budget, or substitution of LLM transcription for dedicated OCR. Fixed
  deployment profile bounds apply to both calls. The description prompt is the same
  for photos, contracts, screenshots and diagrams: describe visual content, layout and
  relationships; separate observation from inference; do not correct or replace OCR.
  The description call receives pixels, not the OCR transcript. Rationale and rejected
  alternatives: `../analysis/media_handling/always_ocr_and_description.md`.
- **One versioned representation, two attributed outputs.** Assemble `document.md` in
  deterministic order: `## Visible text (OCR)` then `## Visual description`. OCR ranges
  retain `ocr` / `source_expression`; description ranges retain the appropriate
  `model_observation` or `model_interpretation` label and their actual component identity.
  ...
  source under D54, even when they repeat the same statement.
- **Failure and replay are per lane.** A successful OCR response with no visible text
  is `empty`, not a coverage failure. A provider/decoder failure never becomes empty
  transcription. Persist successful lane results as internal conversion checkpoints,
  keyed by immutable source identity and the lane's version/configuration fingerprint;
  retry only missing or failed lanes, including after worker restart. Checkpoints are
  bounded, private conversion artifacts covered by source deletion and are not search
  authority. A representation is published only when both required lanes succeed
  (`empty` counts as success); failed processing preserves any previous current
  representation and original access. Record every billed attempt, including rejected
  responses; a crash before checkpoint persistence may cause a repeated provider call
  and must not be represented as an exactly-once guarantee. Profile changes invalidate
  affected checkpoints and produce a new representation through §6. Public reads never
  create these checkpoints. Downstream indexes consume only the completed representation.
...
D116 it opens `<doc_id>/<content_hash>/original.<ext>` directly, which is the better motion
...
successfully detected no text while its description lane succeeded can become
`text_retrieval_ready = true`, with a successful-empty OCR outcome; a failed required lane
prevents publication of that candidate representation under §2; a reader that needs the distinction reads the
...
representation directory, so browsing finds it without any copy being made (D116). The raw
...
| D51 | **confirmed, completed, then amended**: the raw mount + `media/` derived-only rule was the right half; locators + deep links complete "agent gets raw when needed" with second-precision; **D116** withdraws the off-navigation-path clause so a browsing agent reaches the original directly |
...
12. **Image dual-call quality and cost**: measure OCR transcription accuracy, visual
    description fidelity, duplicate-evidence handling and total attempt cost on contracts,
    photos, screenshots and mixed images. Verify no classifier or conditional lane skip.
12b. **Held-out perceptual detail** (the D115 end-to-end check). Build a corpus where each
