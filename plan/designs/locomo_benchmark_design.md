...
> **D119–D121 processing amendments (2026-09-14).** Full-v29 identifies
...
`full-v30-codex-subscription` is an additive evaluator-provider variant for
...
- the non-secret `document_binding_generation`, which Full-v30 requires to be
...
`--protocol` exists only on `prepare`. Canonical runs use `full-v30`; the
explicit `full-v30-gemma-vertex` and `full-v30-codex-subscription` choices are
