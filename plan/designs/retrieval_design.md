...
| `search_documents` | query?, filters? (family, authors, recipients, created/modified ranges, language, thread_ref, doc_ids), k → documents | **find files** (D134, `document_metadata_and_search_design.md` §3): matches file name, title and path by trigram/BM25 and content by the document's overview or summary plus its best chunk, fused by rank; filters on the general document metadata. Each result carries the document, version, general metadata, overview, status, and access handles (P3 path, `source_open`, `data_query`). Several people matching one name are listed, not guessed | — |
| `data_query` | version_id, representation_id?, sql, params? | **the compute-over-this-file operation** (D133, `format_conversion_design.md` §4.6): one read-only SQL statement over a profiled data file's normalized tables, run in DuckDB inside an isolated, resource-capped worker process; returns an `evidence`-grain envelope carrying its own `DataQueryResult/v1` (typed columns, rows, `truncated`, statement hash, and the version, representation and tables read) — not the PostgreSQL-bound open-query `QueryResult/v1`. Runs only where the platform can sandbox the worker without network; otherwise a typed `boundary`. Shares `hydrate depth=bytes`'s authorization and audit. Rows are what memory deliberately does *not* ingest from structured data; this is how an agent answers an aggregation over them. A document without query assets returns a typed `boundary` (§5) | — |
...
`doc_ids`). Chunks match by their document version; claims by their origin chunk's version (the
rule above); relations and observations when at least one live supporting claim comes from a
...
```
{
  ...
  results: [ { …typed record…,
        ...
        provenance: {hydrate_handle, depth_available,
                     ...
                     // D115 — on MEDIA-BEARING items: the compact handle, never content.
  ...
  // D115 — source_open responses only: one entry per native content block, same order, so a
...
`source_open` (D115) resolves the same locator over the same path but delivers the material in
...
- **Media: derivation disclosure and the drop to raw** (D65). Evidence from media carries its
  ...
  through `source_open` (D115), which hands you the source in your own content channels rather
