...
def load_manifest(tier: str) -> QuestionManifest:
    ...
    if manifest.dataset_commit != DATASET_COMMIT:
        raise DatasetValidationError("manifest dataset commit is not RS-LoCoMo-Full-v9")
    if manifest.dataset_sha256 != DATASET_SHA256:
        raise DatasetValidationError("manifest dataset hash is not RS-LoCoMo-Full-v9")
