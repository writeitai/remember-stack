    ...
    *,
    ...
    if outstanding:
        ...
        if build.document_binding_generation != EXPECTED_DOCUMENT_BINDING_GENERATION:
            raise ExecutionGuardError(
                "deployment document binding generation differs from RS-LoCoMo-Full-v28"
            )
    ...
    *,
    ...
    if not _readiness_matches_protocol(
        ...
        raise ExecutionGuardError(
            ...
            " RS-LoCoMo-Full-v28 pipeline, live graph, and fresh P3 projection"
        )
    _require_serving_revision(context=context, readiness=readiness)
...
def _validate_run(
    *,
    ...
    if configuration.dataset_sha256 != DATASET_SHA256:
        raise BenchmarkRunError("run dataset hash is not RS-LoCoMo-Full-v28")
    ...
    if configuration.dataset_commit != DATASET_COMMIT:
        raise BenchmarkRunError("run dataset commit is not RS-LoCoMo-Full-v28")
...
def _require_current_ingest_bindings(*, model_bindings: dict[str, str]) -> None:
    ...
    if model_bindings != expected:
        ...
        raise ExecutionGuardError(
            "deployment ingest model bindings differ from RS-LoCoMo-Full-v28: "
            + ", ".join(mismatches)
        )
